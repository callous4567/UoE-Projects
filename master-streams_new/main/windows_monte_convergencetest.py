...
import galcentricutils_new, hdfutils, windows_directories_new, ascii_info_new
import numpy as np
...
def get_L():
    ...
    tables = [writer.read_table(group, ascii_info_new.set_raw) for group in groups]

    # Grab angular momenta
    tables = [galcentricutils_new.angular().get_momentum(table) for table in tables]
...
def monte_converge(table, table_name, n_max, n_step, avg_length):
    ...
    # Try to save.
    try:
        os.mkdir(windows_directories_new.imgdir + "\\monte_converge")
        plt.savefig(windows_directories_new.imgdir + "\\monte_converge\\" + filename, dpi=300)
    except:
        plt.savefig(windows_directories_new.imgdir + "\\monte_converge\\" + filename, dpi=300)
...
def converge_test():
    ...
    names = ascii_info_new.all_groups
