...
"""
# Important note regarding performance and saving to disk.
"""
Running 25.25x10^6 flips (10,100 sweeps)  takes around 50 minutes on my desktop (processors set to 5.2 GHz)
There is no paralellisation or threading or other fancy stuff, so it may be slow to finish if you intend to run it. 
"""
...
class twod_ising(object):
    def __init__(self):
        ...
        self.dyn = "g"
        # Placeholders for sweeps/equilibration/etc. SEE LINE 23 FOR IMPORTANT INFO REGARDING SWEEPS <> FLIPS.
        self.sweep = 0 # in NUMBER OF FLIPS
        self.equilibration = 2000 # int(0.25e6) # in NUMBER OF FLIPS
        self.measurements = 20000 # int(25e6) # in NUMBER OF FLIPS
        self.max_sweeps = self.equilibration + self.measurements + 1 # in NUMBER OF FLIPS
        self.autocorrelation_length = 25000 # in NUMBER OF FLIPS (useful for average calculations/etc.)
        # Use hard-coded parameters or ask the user? Bit deprecated, given run_high and run_multi.
        self.hard_coded = True
        # User-input timing for console output. Makes output more natural.
        self.delay_max = 5e-3 # for user input typography, delay rate of text
        # For writing the storage file. Overwrite if desired. Group is 'distinct.'
        ...
        # Set image directory. Will only build if main (see: init_multiprocess.)
        ...
        self.binrate = 100 # binrate for animations. Every i'th sweep is selected as frames.
        self.temprate = 4e6 # raise temperature every temprate: only applicable for run_high. TODO: Functional nonlinear
        self.temprise = 0.5 # by this amount: only applicable for run_high
        # Placeholder for dataframe when it's generated.
        self.data = None
        # Save attempts for saving files with self.save. After this, saving will be abandoned (see: problems.)
        self.saveattempts = 5
    ...
    # Kawasaki-dynamic change of the matrix. Magnetisation is constant (thus omitted from notekeeping.)
    def kawasaki(self):
        ...
        # For nearest-neighbours, deal with copying entire matrix (doing consecutive steps.)
        if distance < 1.4:
        ...
        # For non-nearest neighbours (simultaneous glauber.)
        else:
    ...
    def run(self):
        ...
        t1 = ax.text(1, 1, str(self.sweep), color="white", fontsize=5)
        t2 = ax.text(1, self.lx-5, str("T = ") + str(self.T), va='top', color="white", fontsize=5)
        t3 = ax.text(1, self.lx-10, str("M = ") + str(self.M), va='top',  color="white", fontsize=5)
        t4 = ax.text(1, self.lx-15, str("E = ") + str(self.E), va='top',  color="white", fontsize=5)
        ...
        self.data = df
        # Only save if not multiprocessing (i.e. doing a single simple run.)
        if __name__ == "__main__":
            self.save()
    ...
    # Calculate Magnetisation/Energy for each and every sweep in a list, and save it to "all_M" "all_E"
    """
    ...
    # Full run for evaluation of checkpoint TODO: Verify it all works as intended.
    def main(self):
        self.run()
        self.anigen()
    ...
    # Produce graphs for multiprocessed runs selected (see: multirun.py and the relevant simulation ranges.)
    def multigraph(self):
    ...
    # DEPRECATED!!! Keep as code reference.
    ...
    # Create an autocorrelation plot (to estimate autocorrelation time and figure out bad results...)
    # TODO: Figure out why time. We know (sweep-flip) see information at top
