...
class twod_ising(object):
    ...
    ***This should reduce computation time when having to scour for periodic BCs.*** 
    ...
    def averages_errors(self):
        ...
        df = self.data[self.equilibration:self.equilibration + self.measurements]
        # Sample every n'th measurement, autolength from autocorrelation
        autolength = 10
        samplelength = 2 * autolength
        df = df[df.index % samplelength == 0]
        num_samples = len(df)
        ...
        for i in range(number_of_resamples):
            ...
            # Grab Magnetism/Energy samples
            all_M, all_E = all_M[resample], all_E[resample]
            all_MM, all_EE = all_M ** 2, all_E ** 2
            # Get averages
            avg_M, avg_E = np.average(all_M), np.average(all_E)
            avg_MM, avg_EE = np.average(all_MM), np.average(all_EE)
            # Estimate susceptibility and specific heat/spin
            chi = (1/num_samples)*(1/self.T)*(avg_MM - avg_M**2)
            c = (1/num_samples)*(1/self.T**2)*(avg_EE - avg_E**2)
            # Append
            chi_list.append(chi), c_list.append(c)
        chi_list, c_list = np.array(chi_list), np.array(c_list)
        chichi_list, cc_list = chi_list**2, c_list**2
        chi_average, c_average = np.average(chi_list), np.average(c_list)
        chichi_average, cc_average = np.average(chichi_list), np.average(cc_list)
        chi_error, c_error = np.sqrt(chichi_average - chi_average**2), np.sqrt(cc_average - c_average**2)
        # Only save if not multiprocessing
        if __name__ == "__main__":
            ...
            with open(self.imgdir + "\\" + "g.txt", 'wb') as f:
    ...
    def multigraph(self):
        ...
        files = [imgdir + "\\" + "g.txt" for imgdir in imgdirs]
...
#uwu.anigen()
#uwu.autocorrelation()
#uwu.visualize(nns=[25, 0])
#uwu.run_high()
#with open("run_high_dump.txt", 'wb') as f:
#    pickle.dump(obj=uwu, file=f)
#uwu.anigen_high()
