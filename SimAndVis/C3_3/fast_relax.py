...
@njit(fastmath=True)
def fast_jac(rho, pot_dist, shape, boundary, n):

    # Copy
    new_pot_dist = np.empty_like(pot_dist)
...
@njit(fastmath=True)
def potential_stagger(rho, stagger):
                ...
                for ii in range(shape[0]):
                    for jj in range(shape[1]):
                        for kk in range(shape[2]):
                            ...
                            # Distance (skip self-contribution)
                            dist = np.sqrt((ii-i+stagger)**2 + (jj-j+stagger)**2 + (kk-k+stagger)**2)
