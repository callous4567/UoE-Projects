...
#from isochrones.dartmouth import Dartmouth_Isochrone # USING PREVIOUS VERSION V1.2
import isochrones
from TGP2020 import read_mist_models
import random
...
texdir = rootdir + "\\" + "CustomFiguresPGF"
...
class utils(object):
    ...
    def abs_to_app(self, m, m_err, d, d_err):
        M = m - 5 * np.log10(d / 10)
        M_err_Squared = (m_err)**2 + ((5*d_err/d)*(np.log10(np.exp(1))))**2
        M_err = M_err_Squared**0.5
        return M, M_err
    ...
    def stat_reduce(self, name):
        ...
        plt.ylim([0,400])
        plt.title("Background Histogram for " + name)

        plt.savefig(texdir + "\\" + name + "back_raw.png",dpi=300)
        #tikzplotlib.save(texdir + "\\" + name + "back_raw.tex")
        plt.show()
        ...
        plt.title("Background (Smoothed) for " + name)
        plt.xlabel("Pix Value")
        plt.ylabel("Hist Density")
        ...
        plt.savefig(texdir + "\\" + name + "back.png", dpi=300)
        #tikzplotlib.save(texdir + "\\" + name + "back.tex")
        plt.show()
    ...
    def array_gauss_phot(self, data_clip, aprad, apanin, apanout, debug_folder, debug_identifier, save_figs, default_fwhm):
        ...
        if save_figs == True:
            ...
            #tikzplotlib.save(str(debug_identifier) + ".tex")
            plt.close()
    ...
    def standard_fwhmstats(self, bands, fwhms, zeniths, standard):
        ...
        plt.grid(True, which='major', color="pink", alpha=0.25, linestyle='dotted', lw=0.5)  # Enable grids on subplot
        plt.grid(True, which='minor', color="pink", alpha=0.25, linestyle='dotted', lw=0.5)
        plt.xlabel("Passband")
        plt.ylabel("FWHM / Pix")
        ...
        try:
            ...
            tikzplotlib.save(rootdir + "\\" + "Debug" + "\\" + standard + "\\" + standard + "_fwhmfit.tex")
        except:
            pass
...
# Very rough interpolation for the CMD3 isochrones for the purpose of generating mass tracks.
# This is NOT efficient. This is NOT something that will be fast. This is NOT advanced.
# It's rough, linear, and relies on masking of arrays.
# Note that this will not interpolate for isochrones (yet...?)... we just want EEP tracks.
# This has been written in mind of the age steps being 0.01 and the MH being from -0.3 to +0.3 in 0.05 steps.
# This is not a "true" interpolator, but simply made for our purpose.
# table_names_floats = ["Zini", "Mini", "int_IMF",  "Mass"  , "logL"  ,  "logTe",  "logg",  "label",
# "McoreTP", "C_O",  "period0" , "period1",  "period2" , "period3" , "period4" , "pmode" , "Mloss" ,
# "tau1m" ,  "X" ,  "Y" ,  "Xc" , "Xn",  "Xo",  "Cexcess",  "Z" 	, "mbolmag" , "Umag" ,   "Bmag" ,
# "Vmag"  ,  "Rmag"  ,  "Imag"  ,  "Jmag"  ,  "Hmag" ,   "Kmag"]
class cmd3(object):
    def __init__(self, isodir, isofile, isogroup, isoset):
        self.isotable = hdf5_writer(isodir, isofile).read_table(isogroup, isoset)
        self.filer = hdf5_writer(isodir, isofile)
        self.ages = []
        self.float_ages = []
        self.met = 0

    # Will return the astropy table for an isochrone given the AGE and METALLICITY, NO INTERPOLATION
    def isochrone(self, age, met):
        isochrone_by_name = self.isotable.group_by("logAge")
        isomask = isochrone_by_name.groups.keys["logAge"] == age
        masked_isochrone = isochrone_by_name.groups[isomask]
        try:
            masked_by_name = masked_isochrone.group_by("MH")
            mhmask = masked_by_name.groups.keys["MH"] == met
            return masked_by_name.groups[mhmask]
        except:
            print("Isochrone return failed, " + age,met)

    # Will return the isochrone given the current metallicity already being set.
    def isochrone_onlyage(self, age):
        isochrone_by_name = self.isotable.group_by("logAge")
        isomask = isochrone_by_name.groups.keys["logAge"] == age
        masked_isochrone = isochrone_by_name.groups[isomask]
        try:
            return masked_isochrone
        except:
            print("Isochrone return failed, " + age,self.met)

    # Will group the isotable by metallicity + set the metallicity to that provided.
    # Will also group by age.
    def met_age_group(self, met):
        isotable_by_name = self.isotable.group_by("MH")
        isomask = isotable_by_name.groups.keys["MH"] == met
        masked_isotable = isotable_by_name.groups[isomask]
        unique_ages = list(set(masked_isotable['logAge']))
        self.isotable = masked_isotable.group_by("logAge")
        self.ages = unique_ages
        self.met = met
        floatages = [float(d) for d in unique_ages]
        floatages.sort()
        floatages = [("{0:.3f}").format(d) for d in floatages]
        self.float_ages = floatages

    # Will return an interpolated (linearly.) EEP track for the provided mass + metallicity.
    def eep_mass_met(self, mass):
        # This is for each individual mass!
        B_V_list, age_list = [], []
        # Sort by age.
        gc.collect()
        for age_sub in self.ages:
            if float(age_sub) <= 9.25: # just a little limiter for "cleanliness"
                agemask = self.isotable.groups.keys["logAge"] == age_sub
                age_masked = self.isotable.groups[agemask]
                # Use age_masked table, see if mass is inside this tables range. If not, pass.
                if np.min(age_masked['Mass']) < mass < np.max(age_masked['Mass']):
                    # The mass target is indeed inside this range. Next try to estimate the location. Inefficient but whatever.
                    age_masked.sort("Mass", reverse=False)
                    for num, i in enumerate(age_masked):
                        if i['Mass'] >= mass:
                            if np.abs(i['Mass'] - mass) <= 0.2:
                                right, left = num, num - int(1)
                                # With the location, estimate the "fractional" position of the mass from the left element.
                                right_min_left = age_masked[right]['Mass'] - age_masked[left]['Mass']
                                mass_min_left = mass - age_masked[right]['Mass']
                                fract_to_right = mass_min_left/right_min_left
                                # Get the B,V magnitudes either side.
                                colours_left, colours_right = np.array([age_masked[left]["Bmag"], age_masked[left]["Vmag"]]), np.array([age_masked[right]["Bmag"], age_masked[right]["Vmag"]])
                                right_min_left = colours_right - colours_left
                                colours_true = colours_left + (right_min_left*fract_to_right)
                                B_V_list.append(colours_true)
                                age_list.append(age_sub)
                                break
                else:
                    pass
            cleaned_BV, cleaned_age = [], []
            for num, BV in enumerate(B_V_list):
                col = (BV[0] - BV[1])
                if 2.5 >= col >= -0.5:
                    if 15 >= BV[0] >= -5:
                        cleaned_BV.append(BV)
                        cleaned_age.append(age_list[num])
            try:
                np.save(rootdir + "\\EEEPDump\\" + str(mass) + "_" + str(self.met) + ".npy", cleaned_BV)
            except Exception as e:
                print(e)
                print("Failed to save NP ERROR 696969")
...
class fits_alignment(object):
    ...
    def fits_aligner(self, fits1,fits2):
        ...
        # Data is fine until here.
            ...
            plt.xlabel("Pix")
            plt.ylabel("Pix")
            plt.title("ExampleTitle")
            plt.savefig("quiverplot" + fits2split[0] + ".png",dpi=300)
            #tikzplotlib.save("quiverplot" + fits2split[0] + ".tex")
            plt.show()
            ...
            plt.ylabel("Hist Scale")
            plt.savefig("histplot" + fits2split[0] + ".png",dpi=300)
            #tikzplotlib.save("histplot" + fits2split[0] + ".tex")
            plt.show()
            ...
            # Replace failed. Try to make group.
                ...
                # Group exists. Attempt to delete + remake the set
                    ...
                    try:
                        ...
                        print("All writing methods failed... " + str(e) + "..." + group + "_" + dataset)
...
class misc_file_handler(object):
    ...
    # Import CMD .dat isochrone files. Columns listed.
    ...
    # Names are as in file.
    # Metallicities are as in file (as are the logT/etc)
    # Metallicity steps of 0.1 from -0.2 to +0.2, age in steps of 0.02 from 7.6 to 9.28
    def cmd3_import(self, directory, filename, group, set):
        # Grab file.
        file = open(directory + "\\" + filename)
        # Variety of metallicities/ages exist. We'll need to format/create astropy tables for each unique permutation.
        table_names = ["Zini", "MH", "logAge", "Mini", "int_IMF",  "Mass"  , "logL"  ,  "logTe",  "logg",  "label",   "McoreTP", "C_O",  "period0" , "period1",  "period2" , "period3" , "period4" , "pmode" , "Mloss" , "tau1m" ,  "X" ,  "Y" ,  "Xc" , "Xn",  "Xo",  "Cexcess",  "Z" 	, "mbolmag" , "Umag" ,   "Bmag" ,   "Vmag"  ,  "Rmag"  ,  "Imag"  ,  "Jmag"  ,  "Hmag" ,   "Kmag"]
        table_names_floats = ["Zini", "Mini", "int_IMF",  "Mass"  , "logL"  ,  "logTe",  "logg",  "label",   "McoreTP", "C_O",  "period0" , "period1",  "period2" , "period3" , "period4" , "pmode" , "Mloss" , "tau1m" ,  "X" ,  "Y" ,  "Xc" , "Xn",  "Xo",  "Cexcess",  "Z" 	, "mbolmag" , "Umag" ,   "Bmag" ,   "Vmag"  ,  "Rmag"  ,  "Imag"  ,  "Jmag"  ,  "Hmag" ,   "Kmag"]


        dsplits = []
        for d in file:
            d_split = d.split(" ")
            d_split[:] = [item for item in d_split if item != '']
            if d_split[0] != "#":
                try:
                    # 1, 2 = metallicity + logage
                    logt, met = d_split[2], d_split[1]
                    logt_f, met_f = float(logt), float(d_split[1])
                    logt, met = ("{0:.3f}").format(logt_f), ("{0:.1f}").format(met_f) # The new name.
                    d_split[2], d_split[1] = logt, met # Redefine it.
                    dsplits.append(d_split)
                except Exception as e:
                    print(e)
                    print("Finished.")

        dsplits = np.array(dsplits)
        table = Table(dsplits, names=table_names)
        for name in table_names_floats:
            table[name] = [float(d) for d in table[name]]

        filer = hdf5_writer(rootdir, "data.hdf5")
        filer.write_table(group, set, table)

    # Same as above, except it imports a DIRECTORY (titled "isodir" instead of directory, since directory is for the hdf5 file.) of CMD3.2 isochrone files.
    # It vstacks them, It then saves the entire table.
    # The table is saved to a new data.hdf5 file for the sake of not crowding the first, and also since this table will be fucking huge.
    # Make sure only isochrones exist in isodir. There will not be any checks to verify filetype/etc.
    # NOTE FOR MYSELF. 7.45-8.25, 9.05-9.35, steps of 0.01 dex (logt), metallicity from -0.3 to +0.3 in 0.05 dex.
    # Assuming CMD isochrones saved in rootdir/TURNOFF/ISODIR.
    def cmd3_dirport(self, isofilename, isodir):
        cmd_filer = hdf5_writer(self.directory, isofilename) # filer but for the CMD hdf5 file.
        os.chdir(isodir)
        files = os.listdir()
        isoray = []
        table_names = ["Zini", "MH", "logAge", "Mini", "int_IMF", "Mass", "logL", "logTe", "logg", "label", "McoreTP",
                       "C_O", "period0", "period1", "period2", "period3", "period4", "pmode", "Mloss", "tau1m", "X",
                       "Y", "Xc", "Xn", "Xo", "Cexcess", "Z", "mbolmag", "Umag", "Bmag", "Vmag", "Rmag", "Imag", "Jmag",
                       "Hmag", "Kmag"]
        table_names_floats = ["Zini", "Mini", "int_IMF", "Mass", "logL", "logTe", "logg", "label", "McoreTP", "C_O",
                              "period0", "period1", "period2", "period3", "period4", "pmode", "Mloss", "tau1m", "X",
                              "Y", "Xc", "Xn", "Xo", "Cexcess", "Z" , "mbolmag" , "Umag" ,   "Bmag" ,   "Vmag"  ,  "Rmag"  ,  "Imag"  ,  "Jmag"  ,  "Hmag" ,   "Kmag"]
                ...
                for d in f:
                    ...
                    if d_split[0] != "#":
                        try:
                            # 1, 2 = metallicity + logage
                            logt, met = d_split[2], d_split[1]
                            logt_f, met_f = float(logt), float(met)
                            logt, met = ("{0:.3f}").format(logt_f), ("{0:.2f}").format(met_f) # The new name.
                            d_split[2], d_split[1] = logt, met,  # Redefine it.
                            isoray.append(d_split)
                        except Exception as e:
                            print(e)
                            print("Finished " + str(file))

        isoray = np.array(isoray)
        table = Table(isoray, names=table_names)
        for name in table_names_floats:
            table[name] = [float(d) for d in table[name]]

        cmd_filer.write_table("CMD3_Test", "isotable", table)
    ...
    # ColFlags is optional (if it throws an error, ColFlags lets you specifically select columns to use). *Note Coded Yet*
    ...
    def write_votable(self, group, dataset, exportname):
        filer = hdf5_writer(self.directory, self.filename)
        table = filer.read_table(group, dataset)
...
class source_analysis(object):
    ...
    def anasource_manuapertures_redo(self, name, identifier, radec_identifier, aprad, anradin, anradout, region_clip, save_figs):
        ...
        for num,i in enumerate(phottable):
            ...
            # Quick fudge factor to account for the possibility of FWHM being larger in the U than the V. Set to 1.1 FWHMS (roughly how much it varies for the 111773 clips)
            default_fwhm = 1.05*default_fwhm
    ...
    def ana_bicolour(self, group, dataset, band_suffix, band_1_2, autolims, xlims, ylims, titler, absolute_V, dotwidth):
        # Define classes
        filer = hdf5_writer(self.directory, self.filename)
        ...
        #tikzplotlib.save(group + "_" + dataset + ("_{}-{}").format(band_1_2[1], band_1_2[0])+"_BICOLOUR.tex")
    ...
    def ana_tricolour(self, group, dataset, band_suffix, band_1_2_3, autolims, point_label, xlims, ylims, listclip, titler, dotwidth):
        # Define classes
        filer = hdf5_writer(self.directory, self.filename)
        ...
        # Overplot the pleiades stuff. Optional.
        pleitable = filer.read_table("PLEIADES", "phottable")
        plei_UBV = [pleitable['U'], pleitable['B'], pleitable['V']]
        pleiUB, pleiBV = plei_UBV[0] - plei_UBV[1], plei_UBV[1] - plei_UBV[2]
        ax1.scatter(pleiBV, pleiUB, lw=0.5, color='blue', s=0.5)


        # Generate a quick plot of (Av/Etc) for 2nd clust M52
        R = 3.1 # E(B-V) = Av/R
        dydx = 0.74
        sus_dx = -1.9817241379310346/R # Av is something like 2.232
        sus_dy = sus_dx*dydx
        x_values, y_values = x_values + sus_dx, y_values + sus_dy

        ax1.scatter(x=x_values, y=y_values, marker=".", lw=0.2, color="red", s=dotwidth)
        ...
        legend_elements = [Patch(facecolor='black', edgecolor='black', label="Obtained"),
                           Patch(facecolor='blue', edgecolor='black', label="Pleiades")]  # ,
        ...
        #tikzplotlib.save(group + "_" + dataset + "_" + band_suffix + "_TRICOLOUR.tex")
        plt.clf()
        plt.close()
    ...
    # Fancy field plots.
    # Takes astropy table as argument, alongside labels for the X-Band magnitude (absolute, apparent, whatever)
    # we assume ra,dec are just the same as always ("ra", "dec")
    # colourmap should be true or false. By default a heatmap is used (alter this later for customs)
    # minmax is [min_s, max_s], where s = Area of the datapoints
    def ana_fancyplot(self, group, set, magnitude, prov_minz, minmax, titler, numbins, binstep, cminmax, save_identitiy):
        writer = hdf5_writer(rootdir, "data.hdf5")
        data = writer.read_table(group, set)
        print(data.columns)
        x, y = data['ra'], data['dec']
        z = np.array(data[magnitude])

        # We need to bin them. First set up limiters/etc
        if prov_minz == False:
            minz = min(z)
        else:
            minz = prov_minz
        limiters = []
        for num in range(numbins):
            limiters.append(float(minz + num*binstep)) # I.e. if num = 3, min:min+0.5:min+1:min+1.5:everything_else
        limiters.append(99) # Some random limit

        # Bin the data
        raw_list = np.arange(0, numbins, 1)
        xbins, ybins, zbins = [[] for d in raw_list], [[] for d in raw_list], [[] for d in raw_list]
        for num, target_z in enumerate(z):
            for limnum in range(numbins):
                if limiters[limnum] <= target_z < limiters[limnum + 1]:
                    xbins[limnum].append(x[num]), ybins[limnum].append(y[num]), zbins[limnum].append(z[num])

        # Clean up the bin strings (for the sake of plotting/etc... do it to one decimal place.)
        stringed_limiters = [("{0:.1f}").format(d) for d in limiters]

        # Next set up the pointlist for the bins.
        minpoint, maxpoint = minmax
        step = (maxpoint - minpoint)/numbins
        pointlist = []
        for num in range(numbins):
            size = maxpoint - (num + 1)*step
            pointlist.append(size)


        # Set up the figure + etc
        ...
        for limnum in range(numbins):
            if limnum != numbins - 1:
                axs.scatter(x=xbins[limnum], y=ybins[limnum], s=pointlist[limnum], label=(stringed_limiters[limnum] + " " + r'$\leq$' + " " + r'$m_v$' + " " + r'$\leq$' + " " + stringed_limiters[limnum + 1]))
            else:
                                axs.scatter(x=xbins[limnum], y=ybins[limnum], s=pointlist[limnum], label=(
                            stringed_limiters[limnum] + " " + r'$\leq$' + " " + r'$m_V$'), alpha=0.25)
        plt.legend(loc='lower right')

        # Plot/save
        plt.savefig(texdir + "\\" + save_identitiy + "_.png")
        plt.show()
...
# This code is not "automated" like the rest and isn't going to be as cleanly written, be warned.
# Some parts are also dependent on the current project, thus to "reuse" some edits must be made.
class turnoff_analysis(object):
    ...
    # Quick function to generate initial mass function
    # Takes table group, set
    # Takes argument for the absolute luminosity
    # Estimates mass for absolute luminosity + plots a PDMF (present day mass function)
    # The estimate is given as a histogram (with bins)
    def turn_imf(self, group, set, absv, histrange, nbins, alpha, mass_val):
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = filer.read_table(group, set)
        absv_values = table[absv]
        rochester = filer.read_table("MEANSTARS", "spec_table")
        roch_Mass, roch_Mv = rochester['Msun'], rochester['Mv']
        # Map function for Mv to Mass
        mass_funct_absv = interp1d(x=roch_Mv, y=roch_Mass, fill_value="extrapolate")
        mapped_absv_values = [mass_funct_absv(d) for d in absv_values]
        total_mass = np.sum(mapped_absv_values)
        int_total = int(total_mass)

        # Create histogram
        minhist, maxhist = histrange[0], histrange[1]

        histstep = (maxhist - minhist)/nbins
        self.hist_width = histstep
        histvals = [0 for d in range(nbins)]
        histlims = [0]
        histlimmids = []
        for num in range(nbins):
            histlims.append(histlims[num] + histstep)
            for mass in mapped_absv_values:
                if histlims[num] <= mass <= histlims[num + 1]:
                    histvals[num] += 1
            histlimmids.append((histlims[num] + histlims[num + 1])/2)

        # Consider salpeter IMF...
        expconst = -alpha



        # Try to estimate the value of the constant for the salpeter function
        argmax_of_hists = np.argmax(np.array(histvals)) # Using smooths
        massmax_of_hists = histlimmids[argmax_of_hists] # Mass of the maximum value of hists
        histmax_of_hists = histvals[argmax_of_hists] # Number in this bin
        theoretical_constant = (histmax_of_hists/histstep)/((massmax_of_hists**expconst))
        salpeter_funct = lambda mmnought: theoretical_constant * (mmnought ** expconst)
        # Produce the salpeter function
        salpeter = [salpeter_funct(d)*histstep for d in histlimmids]

        # Use theoretical const to estimate total mass (as integral)...
        theory_mass = (theoretical_constant/np.abs(2-alpha))*(mass_val**(2-alpha))
        print(theory_mass)


        fig, axs = plt.subplots(1)
        axs.plot(histlimmids, histvals, color="blue", alpha=0.25)
        axs.plot(histlimmids, salpeter, color="red")
        axs.grid(True, which='major', color="pink", alpha=1, linestyle='dotted', lw=0.5)  # Enable grids on subplot
        axs.grid(True, which='minor', color="pink", alpha=1, linestyle='dotted', lw=0.5)
        axs.set(ylim=[0, np.max(histvals)],
                xlabel=r'$\frac{m}{M_\odot}$',
                ylabel=r'$\xi\left(\frac{m}{M_\odot}\right)\frac{\Delta{m}}{M_\odot}$')

        legend_elements = [Patch(facecolor="red", edgecolor='black', label=("Salpeter " + r'$\alpha=$' + str(alpha))),
                           Patch(facecolor="blue", alpha=0.25, edgecolor='black', label=("Raw Binned Data, Binsize {0:.2f}").format(histstep))]

        axs.legend(handles=legend_elements, loc='upper right')

        plt.savefig(texdir + "\\" + group + "_" + str(int_total) + "_" + str(theory_mass) + ".massplot.png", dpi=300)
        plt.show()

    # Same as above, except it gets the luminosity function.
    def turn_ilf(self, group, set, absv, histrange, nbins):
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = filer.read_table(group, set)
        absv_values = table[absv]
        rochester = filer.read_table("MEANSTARS", "spec_table")
        roch_logL, roch_Mv = rochester['logL'], rochester['Mv']
        # Map function for Mv to Mass
        mass_funct_absv = interp1d(x=roch_Mv, y=roch_logL, fill_value="extrapolate")
        mapped_absv_values = [mass_funct_absv(d) for d in absv_values] # logL for all our Mv
        ...
        # Consider salpeter IMF...
        #expconst = -2.35
        # Smooth down the data a bit
        #histvals_smooth = savgol_filter(histvals, window_length=5, polyorder=1, deriv=0, delta=0, mode='nearest')


        # Try to estimate the value of the constant for the salpeter function
        #argmax_of_hists = np.argmax(np.array(histvals_smooth)) # Using smooths
        #massmax_of_hists = histlimmids[argmax_of_hists] # Mass of the maximum value of hists
        #histmax_of_hists = histvals[argmax_of_hists] # Number in this bin
        #theoretical_constant = (histmax_of_hists/histstep)/((massmax_of_hists**expconst))
        #salpeter_funct = lambda mmnought: theoretical_constant * (mmnought ** expconst)
        ...
        fig, axs = plt.subplots(1)
        axs.plot(histlimmids, histvals, color="blue", alpha=1)
        #axs.plot(histlimmids, salpeter, color="red")
        #axs.plot(histlimmids, histvals_smooth, color="blue")
        axs.grid(True, which='major', color="pink", alpha=1, linestyle='dotted', lw=0.5)  # Enable grids on subplot
        axs.grid(True, which='minor', color="pink", alpha=1, linestyle='dotted', lw=0.5)
        axs.set(ylim=[0, np.max(histvals)],
                xlabel=r'$\log\left[\frac{L}{L_\odot}\right]$',
                ylabel=r'$\epsilon\left(\log\left[\frac{L}{L_\odot}\right]\right)\frac{\Delta{L}}{L_\odot}$')

        legend_elements = [Patch(facecolor="blue", edgecolor='black', label="Luminosity Bins")]

        axs.legend(handles=legend_elements, loc='upper right')

        plt.savefig(texdir + "\\" + group + ".lumplot.png", dpi=300)
        plt.show()

    # Generate absolute V-band magnitudes given the table.
    # We are dealing purely with GAIA crossmatch votables.
    ...
    def turn_bicolour(self, group, dataset, V_ABS, BV_COLOUR, autolims, xlims, ylims, titler, dotwidth):
        ...
        #tikzplotlib.save(group + "_" + dataset + "_" + "VBV_BICOLOUR.tex")

    # Bicolour plotter for provided V_abs/BV colour, that overplots with hipparcos and also our cluster stuff.
    def turn_bicolour_hipparcos(self, group, dataset, V_ABS, BV_COLOUR, autolims, xlims, ylims, titler, dotwidth, clust):
        # Define classes
        filer = hdf5_writer(self.directory, self.filename)
        ...
        # Optionally grab Shazzles example table (debug, hard-coded.) (NGC7789)
        #shazdentifier = clust + "_MEMBERS"
        #shaztable = filer.read_table(shazdentifier, "corrected_ubv")
        #vabsshaz, bvshaz = shaztable['V_abs'], shaztable['BV']

        # Create the plot.
        fig = plt.figure(figsize=(5, 7), dpi=300)
        ax1 = fig.subplots()
        ax1.scatter(x=bv, y=vabs, marker=".", lw=0.1, color="black", s=dotwidth)
        ax1.scatter(x=x_values, y=y_values, marker=".", lw=0.05, color="red", s=2.5*dotwidth)

        #ax1.scatter(x=bvshaz, y=vabsshaz, marker=".", lw=0.05, color="green", s=1.5*dotwidth)
        ...
        fig.show()
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + dataset + "_" + "VBV_HIPBICOLOUR.png")
        #tikzplotlib.save(group + "_" + dataset + "_" + "VBV_HIPBICOLOUR.png")
    ...
    # Takes group, set
    # Takes absV and BV
    # Estimates L(absV) and T(BV)
    # Plots logL vs logT
    # THEORETICAL PHOTOMETRY
    # Weighting for distance vectors included, MIST!!!
    def turn_mspectype_estimator(self, group, set, absV, BV, lims, age_range, increment, maxshift, shift_increment): # age_guess):
        # Grab table of data for processing (i.e. our data)
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = filer.read_table(group, set)
        BV_ori, absV_ori = table[BV], table[absV]
        #BV, absV = table[BV], table[absV]
        #shift = 0.1 # for BV
        #shiftrange = 0.25
        #shifts = np.arange(-shiftrange, shiftrange + 0.01, 0.01)
        #shift_ids = np.arange(0, len(shifts), 1)
        #absV = absV - shift
        #BV = BV - shift/3.1

        # Also load in the MIST catalogues for temp/etc
        os.chdir(self.directory + "\\MistModels")
        # Load ISOCMD/Isochrones
        iso = read_mist_models.ISO(
            "MIST_v1.2_feh_p0.00_afe_p0.0_vvcrit0.0_full.iso")  # 'MIST_v1.2_feh_p0.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd')

        age_guesses = np.arange(age_range[0], age_range[1], increment)
        chisq_mins = []
        for age_guess in age_guesses:
            age_guess_str = ("{0:.2f}").format(age_guess)
            try:
                os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTesting\\" + group)
            except:
                pass
            try:
                os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTesting\\" + group + "\\"+ age_guess_str)
            except:
                pass
            age_ind = iso.age_index(age_guess)
            mistT, mistL = iso.isos[age_ind]['log_Teff'], iso.isos[age_ind]['log_L'] # logarithmic
            mistvects = np.array([mistT, mistL]).T
            mistline = geom.LineString(mistvects)
            chisqs = []

            # Shift range allows under/over estimate of the Av value. Postive shift means overestimate. Negative means underestimate.
            shiftrange = maxshift
            shifts = np.arange(-shiftrange, shiftrange + 0.001, shift_increment)
            shift_ids = np.arange(0, len(shifts), 1)



            for num, shift in enumerate(shifts):
                absV = copy.deepcopy(absV_ori) + shift
                BV = copy.deepcopy(BV_ori) + (shift / 3.1)
                # Get sample table + generate T(BV) function
                sample_table = filer.read_table("MEANSTARS", "spec_table")

                # Teff(BV)
                logTBV = interp1d(sample_table['B-V'], sample_table['logT'], fill_value="extrapolate")
                # logL(Mv)
                logLMv = interp1d(sample_table['Mv'], sample_table['logL'], fill_value="extrapolate")

                # Get logT and logL for all elements of table
                logT = logTBV(BV)
                logL = logLMv(absV)


                # Write to file (Hash out for this part.)
                #table['logT'], table['logL'] = logT, logL
                #filer.write_table(group, set, table)

                # Samples
                loggerT, loggerL = sample_table['logT'], sample_table['logL']

                # Quick plot
                fig, axs = plt.subplots(1)
                axs.scatter(logT, logL, s=0.5, color='red')
                axs.plot(loggerT, loggerL, lw=0.5, color='green')
                axs.plot(mistT, mistL, lw=0.5, color='blue')
                axs.set(title="For " + group,
                        xlabel="logT",
                        ylabel="log" + r'$\frac{L}{L_o}$',
                        xlim = lims[0],
                        ylim = lims[1])
                plt.gca().invert_xaxis()

                legend_elements = [Patch(facecolor='red', edgecolor='black', label="Obt. Fit"),
                                   Patch(facecolor='green', edgecolor='black', label="Rochester"),
                                   Patch(facecolor='blue', edgecolor='black', label="MIST Iso " + age_guess_str)]
                axs.legend(handles=legend_elements, loc='upper right')
                plt.text(4, 0, s = (str(shift) + " =Shift"))
                fig.set_size_inches(10,10)
                plt.show()

                # Generate a leastsq minimization statistic
                vects = np.array([logT, logL]).T
                dists = []


                # DISTANCE WEIGHTINGS!
                dist_weights = 0.1, 0.25, 0.5
                dist_lims = 0, 0.4, 0.8, 100
                lens = len(dist_lims) - 1

                for vect in vects:
                    point = geom.Point(vect)
                    try:
                        distance = (point.distance(mistline)) ** 2
                    except OverflowError:
                        distance = 4

                    for uwa in range(lens):
                        if distance >= dist_lims[uwa]:
                            pass
                        elif distance <= dist_lims[uwa + 1]:
                            dists.append(distance * dist_weights[uwa])

                std = np.std(dists)
                chisq = np.sum([((d/std)**2) for d in dists])
                chisqs.append(chisq)
                ...
                os.chdir(self.directory + "\\Sci\\TURNOFF\\Images\\IsoTesting\\" + group + "\\" + age_guess_str) # dir must be made in main_runtime
                fig.savefig(str(shift_ids[num]) + "_" + age_guess_str + "_" + group + "_" + "loglog" + ".png")
                #tikzplotlib.save(str(shift_ids[num]) + "_" + age_guess_str + "_" + group + "_" + "loglog" + ".tex")

            minchisq = 2*np.min(chisqs)
            chisq_mins.append(minchisq)
            print(minchisq, age_guess)


            # Quick plot
            fig, axs = plt.subplots(1)
            min = np.argmin(chisqs)
            minshift = shifts[min]
            axs.plot(shifts, chisqs, lw=0.5, color='black')
            axs.set(title="For " + group + " with age " + age_guess_str + ", shift = " + str(minshift),
                    xlabel="Av Shift",
                    ylabel="Chisq")
            ...
            os.chdir(self.directory + "\\Sci\\TURNOFF\\Images\\IsoTesting\\" + group) # + "\\" + age_guess_str)  # dir must be made in main_runtime
            fig.savefig("chisq_histogram" + "_" + age_guess_str + "_" + group + "_" + "loglog" + ".png")

        # Quick plot
        fig, axs = plt.subplots(1)
        argmin = np.argmin(chisq_mins)
        age_min = age_guesses[argmin]
        axs.plot(age_guesses, chisq_mins, lw=0.5, color='black')
        axs.set(title="For " + group + ", min = " + str(age_min),
                xlabel="Age Guess",
                ylabel="Chisq Min")
        ...
        os.chdir(
            self.directory + "\\Sci\\TURNOFF\\Images\\IsoTesting\\" + group)  # + "\\" + age_guess_str)  # dir must be made in main_runtime
        fig.savefig("chisq_overall" + "_" + group + "_" + "loglog" + ".png")

    # Same as above but uses synthetic photometry, MIST UBVR(I)
    def turn_mspectype_estimator_UBV(self, group, set, absV, BV, lims, age_range, increment, maxshift, shift_increment): # age_guess):
        # Grab table of data for processing (i.e. our data)
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = filer.read_table(group, set)
        BV_ori, absV_ori = table[BV], table[absV]
        ...
        os.chdir(self.directory + "\\MistModels")
        # Load ISOCMD/Isochrones
        iso = read_mist_models.ISOCMD(
            "MIST_v1.2_feh_p0.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd")  # 'MIST_v1.2_feh_p0.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd')
        ...
        for age_guess in age_guesses:
            age_guess_str = ("{0:.2f}").format(age_guess)
            try:
                os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTestingUBV\\" + group)
            except:
                pass
            try:
                os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTestingUBV\\" + group + "\\"+ age_guess_str)
            except:
                pass
            age_ind = iso.age_index(age_guess)
            synthB, synthV = iso.isocmds[age_ind]['Tycho_B'], iso.isocmds[age_ind]['Tycho_V']
            synthBV = synthB - synthV

            # Convert to Johnson UBV
            V_synth = synthV - 0.090 * (synthBV)
            BV_synth = 0.850 * synthBV
            
            
            mistvects = np.array([BV_synth, V_synth]).T
            mistline = geom.LineString(mistvects)
            chisqs = []
            ...
            for num, shift in enumerate(shifts):
                ...
                # Generate sigma clipped means/etc for the absV/BV for the minimization statistic.

                # Samples
                loggerBv, loggerMv = sample_table['B-V'], sample_table['Mv']

                # Quick plot
                fig, axs = plt.subplots(1)
                axs.scatter(BV, absV, s=0.5, color='red')
                axs.plot(loggerBv, loggerMv, lw=0.5, color='green')
                axs.plot(BV_synth, V_synth, lw=0.5, color='blue')
                axs.set(title="For " + group,
                        xlabel="B-V",
                        ylabel="V",
                        xlim = lims[0],
                ...
                legend_elements = [Patch(facecolor='red', edgecolor='black', label="Obt. Fit"),
                                   Patch(facecolor='green', edgecolor='black', label="Rochester"),
                                   Patch(facecolor='blue', edgecolor='black', label="MIST UBVR(I) " + age_guess_str)]
                axs.legend(handles=legend_elements, loc='upper right')
                plt.text(4, 0, s = (str(shift) + " =Shift"))
                fig.set_size_inches(10,10)
                plt.show()
                ...
                # Generate a leastsq minimization statistic
                vects = np.array([BV, absV]).T
                dists = []
                # DISTANCE WEIGHTINGS! Dist <= 0.1, 0.5, 2, 4
                dist_weights = 1, 1000, 1000
                dist_lims = 0, 0.4, 0.8, 100
                lens = len(dist_lims) - 1
                ...
                os.chdir(self.directory + "\\Sci\\TURNOFF\\Images\\IsoTestingUBV\\" + group + "\\" + age_guess_str) # dir must be made in main_runtime
                fig.savefig(str(shift_ids[num]) + "_" + age_guess_str + "_" + group + "_" + "UBV" + ".png")
                #tikzplotlib.save(str(shift_ids[num]) + "_" + age_guess_str + "_" + group + "_" + "UBV" + ".tex")

            minchisq = np.min(chisqs)
            chisq_mins.append(minchisq)
            ...
            os.chdir(self.directory + "\\Sci\\TURNOFF\\Images\\IsoTestingUBV\\" + group) # + "\\" + age_guess_str)  # dir must be made in main_runtime
            fig.savefig("chisq_histogram" + "_" + age_guess_str + "_" + group + "_" + "UBV" + ".png")
        ...
        os.chdir(
            self.directory + "\\Sci\\TURNOFF\\Images\\IsoTestingUBV\\" + group)  # + "\\" + age_guess_str)  # dir must be made in main_runtime
        fig.savefig("chisq_overall" + "_" + group + "_" + "UBV" + ".png")

    # Same as above but uses synthetic photometry, MIST UBVR(I)
    def turn_mspectype_estimator_UBV_CMD(self, group, set, absV, BV, lims, age_range, increment, metallicity):  # age_guess):
        # Grab table of data for processing (i.e. our data)
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = filer.read_table(group, set)
        BV_ori, absV_ori = table[BV], table[absV]
        vects = np.array([BV_ori, absV_ori]).T # Coordinate points for the BV, V
        dist_errors = table["CMDERR"] # Hardcoded and quickly done, the errors in the above (absolutely, for geometry)

        # Grab isochrone generator/etc for given metallicity
        cmd = cmd3(rootdir, "CMD3.hdf5", "CMD3_Test", "isotable")
        print(metallicity)
        cmd.met_age_group(metallicity)
        print("Grouped by met.")
        # Set up age range/etc + Metallicity Range
        age_guesses = np.arange(age_range[0], age_range[1], increment)
        # Get sample table + generate T(BV) function
        sample_table = filer.read_table("MEANSTARS", "spec_table")
        # Samples
        ...
        for age_guess in age_guesses:
            try:
                age_guess_str = ("{0:.3f}").format(age_guess)
                try:
                    os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTestingUBVCMD\\" + group + "_" + metallicity)
                except:
                    pass
                try:
                    os.mkdir(rootdir + "\\Sci\\TURNOFF\\Images\\IsoTestingUBVCMD\\" + group + "_" + metallicity + "\\" + age_guess_str)
                except:
                    pass

                # Grab isochrone table
                isochrone_table = cmd.isochrone_onlyage(age_guess_str)
                # Grab V, BV
                V_synth, B_synth = isochrone_table['Vmag'], isochrone_table['Bmag']
                BV_synth = B_synth - V_synth
                try:
                    mistvects = np.array([BV_synth, V_synth]).T
                    mistline = geom.LineString(mistvects)
                except:
                    print("Mistline Vect failure for " + group, metallicity, age_guess_str)

                # Quick plot
                fig, axs = plt.subplots(1)
                axs.scatter(BV_ori, absV_ori, s=0.5, color='red')
                axs.plot(loggerBv, loggerMv, lw=0.5, color='green')
                axs.plot(BV_synth, V_synth, lw=0.5, color='blue')
                axs.set(xlabel="B-V",
                        ylabel=r'$M_V$',
                ...
                legend_elements = [Patch(facecolor='red', edgecolor='black', label="Obt. Fit"),
                                   Patch(facecolor='green', edgecolor='black', label="Rochester"),
                                   Patch(facecolor='blue', edgecolor='black', label="CMD3.2 " + age_guess_str)]
                axs.legend(handles=legend_elements, loc='upper right')
                plt.text(4, 0, s=(str(shift) + " =Shift"))
                fig.set_size_inches(10, 10)
                #plt.show()
                ...
                # DISTANCE WEIGHTINGS! Dist <= 0.1, 0.5, 2, 4
                # Final Weights Use 0.2, 0.5, 1
                dist_weights = 0.05, 1 , 1000
                dist_lims = 0, 0.1, 0.3, 100
                lens = len(dist_lims) - 1

                for num, vect in enumerate(vects):
                    point = geom.Point(vect)
                    try:
                        distance = (point.distance(mistline)) ** 2

                        # Divide the distance^2 by the respective error for the chisq sake
                        distance = distance # distance/(dist_errors[num]**2)
                ...
                chisq = np.sum(dists)
                chisq_mins.append(chisq)

                os.chdir(
                    self.directory + "\\Sci\\TURNOFF\\Images\\IsoTestingUBVCMD\\" + group + "_" + metallicity + "\\" + age_guess_str)  # dir must be made in main_runtime
                fig.savefig(age_guess_str + "_" + group + "_" + "UBV" + ".png", dpi=300)
                #tikzplotlib.save(age_guess_str + "_" + group + "_" + "UBV" + ".tex")
                plt.close()
                plt.clf()
            except:
                chisq_mins.append(2000)
                pass
        # Quick plot
        fig, axs = plt.subplots(1)
        argmin = np.argmin(chisq_mins)
        age_min = age_guesses[argmin]
        chisqmin = chisq_mins[argmin]
        axs.plot(age_guesses, chisq_mins, lw=0.5, color='black')
        axs.set(xlabel="Age Guess",
                ylabel="Chisq Min")

        axs.axhspan(chisqmin, chisqmin + 1, facecolor='green', alpha=0.5)
        axs.axhspan(chisqmin + 1, chisqmin + 2, facecolor='blue', alpha=0.5)
        axs.axhspan(chisqmin + 2, chisqmin + 3, facecolor='red', alpha=0.5)


        coord_of_text = [np.min(age_guesses) + 0.1, np.min(chisq_mins) + 1]
        age_guess_str = ("{0:.3f}").format(age_min)
        plt.text(coord_of_text[0], coord_of_text[1], s=age_guess_str)
        fig.set_size_inches(10, 10)
        #plt.show()

        os.chdir(
            self.directory + "\\Sci\\TURNOFF\\Images\\IsoTestingUBVCMD\\" + group + "_" + metallicity)  # + "\\" + age_guess_str)  # dir must be made in main_runtime
        fig.savefig("chisq_overall" + "_" + group + "_" + "UBV" + ".png", dpi=300)
        plt.close()
        plt.clf()
        #tikzplotlib.save("chisq_overall" + "_" + group + "_" + "UBV" + ".tex")

        # Also save the plot for future analysis (if wanted)
        filer = hdf5_writer(rootdir, "data.hdf5")
        table = Table()
        table['ages'] = age_guesses
        table['chisqs'] = chisq_mins
        filer.write_table(group, metallicity, table)

    # Tries to fit isochrone to curves given.
    def isofit(self, group, set, V, BV, age_guess, lims):
        filer = hdf5_writer(self.directory, self.filename)
        # Load data
        table = filer.read_table(group, set)
        V, BV = table[V], table[BV]
        shift = 0
        BV = BV + shift
        coords = np.array([BV, V])
        vects = coords.T
        os.chdir(self.directory + "\\MistModels")
        # Load ISOCMD/Isochrones
        iso = read_mist_models.ISOCMD('MIST_v1.2_feh_p0.00_afe_p0.0_vvcrit0.0_UBVRIplus.iso.cmd')
        time.sleep(50)
        age_ind = iso.age_index(age_guess)
        synthB, synthV = iso.isocmds[age_ind]['Tycho_B'], iso.isocmds[age_ind]['Tycho_V']
        synthBV = synthB - synthV
        ...
        # Set up list. Using Tycho mag.
        print("HDR LIST", iso.hdr_list)

        """
        # Proceed to produce chi_sq plot for age_guess in increments of 0.1, range of +- 1 order of magnitude.
        age_min, age_max = age_guess - 0.5, age_guess + 0.5
        values = np.arange(age_min, age_max, 0.025)
        chisq_un_norm = []
        for value in values:
            age_ind = iso.age_index(value)
            synthB, synthV = iso.isocmds[age_ind]['Tycho_B'], iso.isocmds[age_ind]['Tycho_V']
            synthBV = synthB - synthV
            ...
            V_synth = synthV - 0.090 * (synthBV)
            BV_synth = 0.850 * synthBV
            vects_synth = np.array([BV_synth, V_synth]).T
            line_synth = geom.LineString(vects_synth)
            dist_lim = []
            dist_all = []

            for vect in vects:
                point = geom.Point(vect)
                distance = point.distance(line_synth)
                dist_all.append(distance)
                dist_lim.append(distance)

            mean, med, std = sigma_clipped_stats(dist_all)
            dist_lim = np.array(dist_lim)
            chisquarer = dist_lim / mean
            dist_lim = np.sum((np.array(chisquarer))**2)
            chisq_un_norm.append(dist_lim)


            # Plotting/etc
            fig, axs = plt.subplots(1)
            axs.scatter(BV, V, color="red", marker='x', s=0.05)
            axs.plot(BV_synth, V_synth, color='green')
            axs.set(xlim=lims[0],
                    ylim=lims[1],
                    title="Isochrone (UBVR(i) Combined, MIST V1.2) fit for " + group,
                    xlabel="B - V",
                    ylabel="V/Abs")
            #plt.text(lims[0][0] + 0.05, lims[1][0] + 0.5, "Invsq is " + str(dist_lim))

            legend_elements = [Patch(facecolor='red', edgecolor='black', label="Data"),
                               Patch(facecolor='green', edgecolor='black',
                                     label=("log10(age/yr) = {}").format(value))]
            axs.legend(handles=legend_elements, loc='upper right')
            ...
            plt.show()
            os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
            fig.savefig(group + "_" + set + "_" + "isofit_CUSTOM_" + str(value) + ".png")
            plt.close()

        # Minargs/etc
        argmax = np.argmax(chisq_un_norm)
        valmax = values[argmax]
        agemin_gyr = 10**valmax / (1e9)
        fig, axs = plt.subplots(1)
        axs.plot(values, chisq_un_norm, lw=0.5, color='black')
        axs.set(xlabel="log10(age/yr",
                ylabel="Inv-sq",
                title="Inv-sq for isochrone, " + group)
        plt.text(min(values) + 0.1, min(chisq_un_norm) + 5, "Invsq age is " + str(agemin_gyr) + "/Gyr")
        plt.show()
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_" + "isofit_chisq" + ".png")
        print("All saved, babe.")
        ...
        legend_elements = [Patch(facecolor='red', edgecolor='black', label="Data"),
                           Patch(facecolor='green', edgecolor='black', label=("log10(age/yr) = {}").format(age_guess))]
        axs.legend(handles=legend_elements, loc='upper right')
        ...
        plt.show()
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_" + "isofit_CUSTOM.png")
        #tikzplotlib.save(group + "_" + set + "_" + "isofit_CUSTOM.tex")

        # DARTMOUTH FIT USING OLD VERSION (V1.2) not good enough for M52.
        """
        filer = hdf5_writer(self.directory, self.filename)
        table = filer.read_table(group, set)
        V, BV = table[V], table[BV]
        iso = Dartmouth_Isochrone(['B', 'V'])
        model = iso.isochrone(age_guess)
        modb, modv = model.B_mag, model.V_mag
        modbv = modb - modv
        fig, axs = plt.subplots(1)
        axs.scatter(BV, V, color="red", marker='x', s=0.05)
        axs.plot(modbv, modv, color='green')
        axs.set(xlim=lims[0],
                ylim=lims[1],
                title="Isochrone fit for " + group,
                xlabel="B - V",
                ylabel="V/Abs")
        ...
        plt.show()
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_" + "isofit_DARTHMOUTH.png")"""

    # Plots a CMD isochrome
    def isoCMDfit(self, group, set, V, BV, age_guess, metallicity, lims):
        # "CMD3", "isotable"
        filer = hdf5_writer(self.directory, self.filename)
        # Load data
        table = filer.read_table(group, set)
        ...
        # Load in isochromes
        isochrone = filer.read_table("CMD3", "isotable")

        # Grab group logAge MH = logt/metallicity column names.
        # Group first by the age, then by Z.
        isochrone_by_name = isochrone.group_by("logAge")
        isomask = isochrone_by_name.groups.keys["logAge"] == age_guess
        masked_isochrone = isochrone_by_name.groups[isomask]
        masked_by_name = masked_isochrone.group_by("MH")
        mhmask = masked_by_name.groups.keys["MH"] == metallicity
        mh_final = masked_by_name.groups[mhmask]


        # Grab B/V
        V_synth, B_synth = mh_final['Vmag'], mh_final['Bmag']
        BV_synth = B_synth - V_synth
        new_Vsynth, newBV_synth = [],[]
        for num, vs in enumerate(V_synth):
            if vs <= 6:
                new_Vsynth.append(vs)
                newBV_synth.append(BV_synth[num])

        V_synth, BV_synth = new_Vsynth, newBV_synth
        ...
        axs.set(xlim=lims[0],
                ylim=lims[1],
                title="Isochrone PARSEC CMD3.2 " + group,
                xlabel="B - V",
                ylabel="V/Abs")

        legend_elements = [Patch(facecolor='red', edgecolor='black', label="Data"),
                           Patch(facecolor='green', edgecolor='black', label=("log10(age/yr), [Fe/H] = {},{}").format(age_guess, metallicity))]
        axs.legend(handles=legend_elements, loc='upper right')

        major_ticks_x, major_ticks_y, minor_ticks_x, minor_ticks_y = 1, 1, 0.2, 0.2  # Set your majors and minors. It might be worth modifying this if you intend to clip graphs along the y axis.
        ...
        axs.xaxis.set_major_locator(major_ticks_x)  # Apply locators to ax subplot
        axs.yaxis.set_major_locator(major_ticks_y)
        axs.xaxis.set_minor_locator(minor_ticks_x)
        axs.yaxis.set_minor_locator(minor_ticks_y)
        ...
        plt.show()
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_" + "isofitCMD3.2_CUSTOM.png")
        #tikzplotlib.save(group + "_" + set + "_" + "isofitCMD3.2_CUSTOM.tex")
...
class member_finder(object):
    ...
    # Also estimate in pmra/pmdec space.
    def pmscat(self, group, set, pmra, pmdec, lims, titler):
        table = self.filer.read_table(group, set)
        # Tables
        x, y = table[pmra], table[pmdec]
        x, y = [float(d) for d in x], [float(d) for d in y]
        ...
        # Meshgrid for density estimate. Start with 50 points along each axis. *100*. Change RESOLUTION too!!!!
        NPoints = 100j
        xmin,xmax,ymin,ymax = lims[0][0], lims[0][1], lims[1][0], lims[1][1]
        resolutionx = (xmax - xmin) / 100
        resolutiony = (ymax - ymin) / 100
        xx, yy = np.mgrid[xmin:xmax:NPoints, ymin:ymax:NPoints]

        # Fit gaussian
        pos, dat = np.vstack([xx.ravel(), yy.ravel()]), np.vstack([x,y])
        kern = st.gaussian_kde(dat, bw_method=0.05)
        functer = np.reshape(kern(pos).T, xx.shape)
        maxi = np.argmax(functer)
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        self.filer.write(group, "centroid_pm", actu_coord)


        # Get radii about centroid
        radius = []
        for num, xcord in enumerate(x):
            radius_square = (actu_coord[0] - xcord)**2 + (actu_coord[1] - y[num])**2
            radius.append(np.sqrt(radius_square))

        # Produce radial density distribution.
        radius_of_plot = 0.5 # Max radius to allow for plotting.
        ann_number = 15 # Number of annuli
        ann_indices = np.arange(0, ann_number, 1)
        r_limiters = []
        # Generate radii
        for d in range(ann_number + 1):
            r_limiters.append((d) * radius_of_plot/ann_number)
        # Bin radii
        tables = np.arange(0, ann_number, 1)
        for rad in radius:
            for index in ann_indices:
                if rad <= r_limiters[index + 1]:
                    tables[index] += 1
                    break
        # Generate normalized distribution
        densities = []
        radii = []
        for d in range(ann_number):
            area = np.pi*(r_limiters[d + 1]**2 - r_limiters[d]**2)
            densities.append(tables[d]/area)
            radii.append((r_limiters[d + 1] + r_limiters[d])/2)

        fig2, axs2 = plt.subplots(1)
        axs2.plot(radii, densities)
        plt.show()
        plt.close()
        plt.clf()
        ...
        cfset = ax.contourf(xx, yy, functer, cmap='coolwarm')
        ax.imshow(np.rot90(functer), cmap='coolwarm', extent=[xmin, xmax, ymin, ymax], aspect='auto')
        cset = ax.contour(xx, yy, functer, colors='k')
        #ax.clabel(cset, inline=1, fontsize=10)

        ax.scatter(x, y, s=0.5, color="black")
        ax1 = ax.scatter(actu_coord[0], actu_coord[1], marker='x', color="white", s=40)
        ax.set(xlabel="pm " + r'$\alpha$' + " / mas/Yr",
               ylabel="pm " + r'$\delta$' + " / mas/Yr",
               xlim=lims[0],
               ylim=lims[1],
               title="pm " + r'$\alpha,\delta$' + " KDE Estimation for " + titler)
        ax.legend([ax1], ["Centroid = " + ("({0:.2f}, {1:.2f})").format(actu_coord[0], actu_coord[1])], loc='upper right')
        ...
        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "pmscat.png", dpi=300)
        #tikzplotlib.save(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "pmscat.tex")
        plt.show()

    # Same as pmscat but for ra/dec
    def radecscat(self, group, set, ra, dec, lims, titler):
        table = self.filer.read_table(group, set)
        # Tables
        ...
        # Fit gaussian
        pos, dat = np.vstack([xx.ravel(), yy.ravel()]), np.vstack([x,y])
        kern = st.gaussian_kde(dat, bw_method=0.25)
        functer = np.reshape(kern(pos).T, xx.shape)
        maxi = np.argmax(functer)
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        self.filer.write(group, "centroid", actu_coord)
        ...
        ax.set(xlabel=r'$\alpha$' + " / deg",
               ...
               xlim=lims[0],
               ylim=lims[1],
               title=r'$\alpha,\delta$'+ " KDE Estimation for " + titler)
        ax.legend([ax1], ["Centroid = " + ("({0:.2f}, {1:.2f})").format(actu_coord[0], actu_coord[1])], loc='upper right')
        ...
        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "radecscat.png", dpi=300)
        #tikzplotlib.save(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "radecscat.tex")
        plt.show()

    # Given group get radial info/plots. centroid/centroid_pm are coordinates in spaces respective.
    # Will alter original table + add in radial distance (in mas, analogous to proper motion.)
    def radcalc(self, group, set, rad_tolerance, pm_tolerance, mag_tolerance):
        radec_cent = self.filer.read(group, "centroid")
        radec_cent_rad = [np.deg2rad(d) for d in radec_cent]
        pm_cent = self.filer.read(group, "centroid_pm")
        table = self.filer.read_table(group, set)
        reduced_table = Table(names=table.colnames)
        radius, pmradius = [], []
        distances = []
        for i in table:
            # Conventional radius.
            ...
            radif, decdif = rarad - radec_cent_rad[0], decrad - radec_cent_rad[1]
            angular_dif = np.sqrt((radif*np.cos(decrad))**2 + decdif**2)
            angular_dif_arcmin = self.utilities.ra_arcmin(angular_dif)
            radius.append(angular_dif_arcmin)

            # PM Space radius
            pmra, pmdec = i['pmra'], i['pmdec'] # in mas/year
            pmdif_mag = np.sqrt((pmra - pm_cent[0])**2 + (pmdec - pm_cent[1])**2)
            magdif_mag = i['V_apflux_annuli_manu_atmos_scimag'] - i['phot_bp_mean_mag']

            # Also limit in distance-space.
            parallax = i['parallax']
            distance = self.utilities.par_to_dist(parallax, 0)

            # Reduced table.
            if angular_dif_arcmin <= rad_tolerance:
                if pmdif_mag <= pm_tolerance:
                    if np.abs(magdif_mag) <= mag_tolerance:
                        distances.append(distance[0])
                        reduced_table.add_row(i)

        mean, med, std = sigma_clipped_stats(distances)
        reduced_table['dist'] = distances
        red_reduced_table = Table(names=reduced_table.columns)
        # Will also reduce table based on sigma-clipped parallax distances.
        for i in reduced_table:
            if np.abs(i['dist'] - mean) <= 2*std:
                red_reduced_table.add_row(i)

        # Save reduced table
        self.filer.write_table(group, set + "_reduced", red_reduced_table)

        # Produce radial density distribution.
        radius_of_plot = 25 # Max radius to allow for plotting.
        ann_number = 15 # Number of annuli
        ann_indices = np.arange(0, ann_number, 1)
        r_limiters = []
        # Generate radii
        ...
        # Smooth densities before plotting.
        densities2 = savgol_filter(densities, window_length=1, polyorder=0, deriv=0, delta=0, mode='nearest')
        dens_funct = interp1d(radii, densities2, fill_value='extrapolate')
        radii = np.linspace(0, 25, 500)
        dens = dens_funct(radii)

        fig, axs = plt.subplots(2, sharex=True)
        axs[0].plot(radii, dens)
        axs[0].set(ylabel="Density/arcsec**2",
                   title="Density plot for " + group)
        axs[1].hist(radius)
        axs[1].set(xlabel="Radius/arcsec",
                   ylabel="Density/arcsec",
                   title="Separation histogram for " + group)
        fig.set_size_inches(8,10)
        plt.show()

        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_" + "densities.png")
        #tikzplotlib.save(group + "_" + set + "_" + "densities.tex")

        # Take table (group, set) and create proper motion scatter plots.
        # Also estimate in pmra/pmdec space.
        def pmscat(self, group, set, pmra, pmdec, lims):
            table = self.filer.read_table(group, set)
            # Tables
            x, y = table[pmra], table[pmdec]
            x, y = [float(d) for d in x], [float(d) for d in y]
            ...
            # cset = ax.contour(xx, yy, functer, colors='k')
            # ax.clabel(cset, inline=1, fontsize=10)
            plt.title("2D Gaussian Kernel density estimation for " + group)

            ax.scatter(x, y, s=0.5, color="black")
            ax.scatter(actu_coord[0], actu_coord[1], marker='x', color="white", s=40)
            ax.set(xlabel="pmra/m''",
                   ylabel="pmdec/m''",
                   title="PM Scatter for " + group,
                   xlim=lims[0],
                   ylim=lims[1])
            plt.text(xmin + 0.1, ymin + 0.1, s="Centroid = " + str(actu_coord), color="white")
        ...
        # Same as pmscat but for ra/dec
        def radecscat(self, group, set, ra, dec, lims):
            table = self.filer.read_table(group, set)
            # Tables
            x, y = table[ra], table[dec]
            x, y = [float(d) for d in x], [float(d) for d in y]
            ...
            ax.scatter(x, y, s=0.5, color="black")
            ax.scatter(actu_coord[0], actu_coord[1], marker='x', color="white", s=40)
            ax.set(xlabel="ra/deg",
                   ylabel="dec/deg",
                   title="RA/DEC Scatter for " + group,
                   xlim=lims[0],
                   ylim=lims[1])
            plt.text(xmin + 0.1, ymin + 0.1, s="Centroid = " + str(actu_coord), color="white")
    ...
    # This is just a quick bit of code to try for the 2nd cluster in M52 images. Nothing fancy.
    def m52second(self, group, set, rad_tolerance, pm_tolerance, mag_tolerance):
        radec_cent = self.filer.read(group, "centroid_second") # By-eye guess.
        radec_cent_rad = [np.deg2rad(d) for d in radec_cent]
        pm_cent = self.filer.read(group, "centroid_pm_second")
        table = self.filer.read_table(group, set)
        reduced_table = Table(names=table.colnames)
        radius, pmradius = [], []
        distances = []
        for i in table:
            ...
            # Reduced table. Distance is further limited. ARBITRARY!!!!
            if angular_dif_arcmin <= rad_tolerance:
                if pmdif_mag <= pm_tolerance:
                    if np.abs(magdif_mag) <= mag_tolerance:
                        if np.abs(distance[0] - 2840) <= 100:
                            distances.append(distance[0])
                            reduced_table.add_row(i)
        ...
        # Will also reduce table based on sigma-clipped parallax distances.
        for i in reduced_table:
            if np.abs(i['dist'] - mean) <= 4*std:
                red_reduced_table.add_row(i)

        # Quickly get an error on distance
        utilities = utils()
        disterrs = []
        for i in red_reduced_table:
            disterr = utilities.par_to_dist(i['parallax'], i['parallax_error'])
            disterrs.append(disterr[1])
        red_reduced_table['dist_err'] = disterrs
        print(np.mean(red_reduced_table['dist']), np.mean(red_reduced_table['dist_err']))
        print(np.std(red_reduced_table['dist']))


        # Save reduced table
        self.filer.write_table(group, set + "_second_reduced", red_reduced_table)
        ...
        axs[0].plot(radii, dens)
        axs[0].set(ylabel="Density/arcsec**2",
                   title="Density plot for second cluster " + group)
        axs[1].hist(radius)
        axs[1].set(xlabel="Radius/arcsec",
                   ylabel="Density/arcsec",
                   title="Separation histogram for second cluster " + group)
        fig.set_size_inches(8,10)
        plt.show()

        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_second_" + "densities")

    # MODIFIED FOR SECOND CLUSTER.
    # Also estimate in pmra/pmdec space.
    def pmscat2(self, group, set, pmra, pmdec):
        table = self.filer.read_table(group, set)
        # Tables
        x, y = table[pmra], table[pmdec]
        x, y = [float(d) for d in x], [float(d) for d in y]
        ...
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        self.filer.write(group, "centroid_pm_second", actu_coord)
        ...
        ax.set(xlabel="pmra/m''",
               ylabel="pmdec/m''",
               title="PM Scatter for [FSR 2007] 433",
               xlim=lims[0],
               ylim=lims[1])
        plt.text(xmin + 0.1, ymin + 0.1, s="Centroid = " + str(actu_coord), color="white")
        ...
        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "pmscat_second.png", dpi=300)
        plt.show()

    # Same as pmscat but for ra/dec
    def radecscat2(self, group, set, ra, dec, ):
        table = self.filer.read_table(group, set)
        # Tables
        x, y = table[ra], table[dec]
        x, y = [float(d) for d in x], [float(d) for d in y]
        ...
        # Catch lims
        # radec_cent = [351.45, 61.3478]
        lims = [[351.2, 351.7], [61.2, 61.45] ]
        ...
        NPoints = 50j
        xmin,xmax,ymin,ymax = lims[0][0], lims[0][1], lims[1][0], lims[1][1]
        resolutionx = (xmax - xmin) / 50
        resolutiony = (ymax - ymin) / 50
        xx, yy = np.mgrid[xmin:xmax:NPoints, ymin:ymax:NPoints]

        # Fit gaussian
        pos, dat = np.vstack([xx.ravel(), yy.ravel()]), np.vstack([x,y])
        kern = st.gaussian_kde(dat, bw_method=0.2)
        functer = np.reshape(kern(pos).T, xx.shape)
        maxi = np.argmax(functer)
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        self.filer.write(group, "centroid_second", actu_coord)
        ...
        ax.set(xlabel="ra/deg",
               ylabel="dec/deg",
               title=(r'$\alpha,\delta$' + " KDE for [FSR 2007] 433"),
               xlim=lims[0],
               ylim=lims[1])
        plt.text(xmin + 0.01, ymin + 0.01, s="Centroid = " + str(actu_coord), color="white")

        ax.grid(True, which='major', color="blue", alpha=1, linestyle='dotted', lw=0.5)  # Enable grids on subplot
        ax.grid(True, which='minor', color="pink", alpha=1, linestyle='dotted', lw=0.5)


        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "radecscat_second.png", dpi=300)
        plt.show()

    # And these are specifically for using GAIA obtained data directly.
    def m52third(self, group, set, rad_tolerance, pm_tolerance, mag_tolerance):
        radec_cent = self.filer.read(group, "centroid_second") # By-eye guess.
        radec_cent_rad = [np.deg2rad(d) for d in radec_cent]
        pm_cent = self.filer.read(group, "centroid_pm_second")
        table = self.filer.read_table(group, set)
        ...
        os.chdir(self.directory + "\\Sci\\TURNOFF\\Images")
        fig.savefig(group + "_" + set + "_second_GAIA" + "densities")

    # MODIFIED FOR SECOND CLUSTER.
    # Also estimate in pmra/pmdec space.
    def pmscat3(self, group, set, pmra, pmdec):
        table = self.filer.read_table(group, set)
        # Tables
        x, y = table[pmra], table[pmdec]
        x, y = [float(d) for d in x], [float(d) for d in y]
        ...
        # Fit gaussian
        pos, dat = np.vstack([xx.ravel(), yy.ravel()]), np.vstack([x,y])
        kern = st.gaussian_kde(dat, bw_method=0.03)
        functer = np.reshape(kern(pos).T, xx.shape)
        maxi = np.argmax(functer)
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        ...
        ax.set(xlabel="pmra/m''",
               ylabel="pmdec/m''",
               title="PM Scatter for 2nd Clust" + group,
               xlim=lims[0],
               ylim=lims[1])
        plt.text(xmin, ymin, s="Centroid = " + str(actu_coord), color="white")

        ax.grid(True, which='major', color="blue", alpha=1, linestyle='dotted', lw=0.5)  # Enable grids on subplot
        ax.grid(True, which='minor', color="pink", alpha=1, linestyle='dotted', lw=0.5)


        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "pmscat_second_GAIA.png", dpi=300)
        plt.show()

    # Same as pmscat but for ra/dec
    def radecscat3(self, group, set, ra, dec, ):
        table = self.filer.read_table(group, set)
        # Tables
        x, y = table[ra], table[dec]
        x, y = [float(d) for d in x], [float(d) for d in y]
        ...
        # Catch lims
        # radec_cent = [351.45, 61.3478]
        lims = [[351.3, 351.6], [61.275, 61.375] ]
        ...
        # Fit gaussian
        pos, dat = np.vstack([xx.ravel(), yy.ravel()]), np.vstack([x,y])
        kern = st.gaussian_kde(dat, bw_method=0.08)
        functer = np.reshape(kern(pos).T, xx.shape)
        maxi = np.argmax(functer)
        actu_max = np.unravel_index(maxi, functer.shape)
        actu_coord = actu_max[0]*resolutionx + xmin, actu_max[1]*resolutiony + ymin # Centroid of KDE (roughly)
        ...
        ax.set(xlabel="ra/deg",
               ylabel="dec/deg",
               title="RA/DEC Scatter for 2nd Clust" + group,
               xlim=lims[0],
               ylim=lims[1])
        plt.text(xmin, ymin, s="Centroid = " + str(actu_coord), color="white")
        ...
        plt.savefig(rootdir + "\\Sci\\TURNOFF\\Images\\" + group + "_" + "radecscat_second_GAIA.png", dpi=300)
        plt.show()
...
#votableha.plei_import("Melotte22PLEIADES.csv", "PLEIADES", "phottable")
#votableha.write_votable("M52_PREREDCATALOGUE", "phottable_table_prered", "M52_FINAL.vot")
#votableha.write_votable("NGC7789_PREREDCATALOGUE", "phottable_table_prered", "NGC7789_FINAL.vot")
#votableha.write_votable("M52_Cross", "raw_data_reduced", "M52_Redu.vot")
#votableha.write_votable("NGC7789_Cross", "raw_data_reduced", "NGC7789_Redu.vot")
#votableha.write_votable("M52_Cross", "raw_data_second_reduced", "Second_Clust.vot")
#votableha.cmd3_dirport("CMD3.hdf5", rootdir + "\\Sci\\TURNOFF\\CMD3")

#handler = misc_file_handler(rootdir, "data.hdf5")
#handler.cmd3_import(rootdir, "output753146698306.dat", "CMD3", "isotable")

#utilities = utils()
#utilities.stat_reduce("PIRATE_323754_OSL_ROE_SC2_NGC7789_cluster_00_ObjectName_00_Filter_U_00_2020_10_11_22_49_49.fits")
"""
writer = hdf5_writer(rootdir, "data.hdf5")
tables = [writer.read_table("M52_Cross", "raw_data"), writer.read_table("NGC7789_Cross", "raw_data")]
for table in tables:
    dif_vgbp = table['V_apflux_annuli_manu_atmos_scimag'] - table['phot_bp_mean_mag']
    dif_vgbp = dif_vgbp[0:400]
    corrected = []
    for d in dif_vgbp:
        if np.isnan(d) != True:
            corrected.append(d)
    stdev = np.std(corrected)
    print(stdev)"""
