...
class particle(object):
    ...
    # FILE READER.
    # Instantiate a particle, i.e. hey = randparticlearray(), heyo = hey.generator(1).
    # Once instantiated, do heyo.usefile(nametension, timestep)
    # Takes arguments:
    # - NAMETENSION... the name of the file and the extension, i.e. hey.dat
    # - TIMESTEP... the timestep involved for any integrators.
    # FILE FORMAT
    # X Y Z VX VY VZ MASS INDEX
    # The exercise wants a static method to read data from a file... while I think it's a bit nuts to have a single lined file for a particle object,
    # UNTESTED FEATURE THAT I JUST ADDED IN. hopefully it works :3
    def usefile(self, nametension, timestep):
        file = open(nametension, "r")
        liner = file.readlines()
        linered = liner[0].split(" ")
        self.pos = np.array([float(linered[0]), float(linered[1]), float(linered[2])])
        self.vel = np.array([float(linered[3]), float(linered[4]), float(linered[5])])
        self.mass = float(linered[6])
        self.index = str(linered[7])
        self.step = timestep
        file.close()
...
# The stack of debug stuff. Ignore this.
"""
hey = randparticlearray(4)
heyo = hey.array_gen()
print(hey.array_gen())
print(heyo[0][0])
"""
"""
babe = filereader()
hey = babe.reader_former("filetest.dat", 4, 0.01)
print(hey)
print(hey[0])
print(hey[0][1])
"""
...
"""
parti_1 = particle(np.array([2,3,4]), [0,0,0], 1, 2, 3)
parti_2 = particle(np.array([3,4,5]), [0,0,0], 1, 2, 3)
p3 = parti_2 - parti_1
print(p3)
"""