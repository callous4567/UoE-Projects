...
class moon_solver(object):
    ...
    def mainethestate(self):
        self.relvects()
        lunasolve = ellipse_solver(self.relvectata, self.timestep, "Luna.txt", 2, 15)
        lunasolve.main()
...
# This loop is optimised for timestep of 0.1, 1 million steps, custom for Halley, Earth and Moon (Halley is problematic.)

for d in range(len(filedatalist)):
    if filedatalist[d] == "Sun.dat":
        pass
    elif filedatalist[d] == "Halley.dat":
        solver_object = ellipse_solver(import_tool.orbit_porter(filedatalist[d]), timestep, filedatalist[d], 15000, 100000)
        solver_object.main()
    else:
        solver_object = ellipse_solver(import_tool.orbit_porter(filedatalist[d]), timestep, filedatalist[d], 30, 200)
        solver_object.main()
