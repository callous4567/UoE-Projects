...
# FROM HEREON ALL THE CODE WILL BE DIRTY AS FUCK AND WON'T BE INTENDED TO BE REPRODUCEABLE. SORRY NOT SORRY!
    ...
    def source_diagrams(self):
        ...
        # sourcer.ana_bicolour("GAIA", "test_query", "", ["g","bp"], False, False, [-1.5, 4], [23, 9], "GAIA bp-g CMD for M52", False, 1)
        sourcer.ana_bicolour("M52_PREREDCATALOGUE", "phottable_table_prered", "_apflux_annuli_manu_atmos_scimag", ["V","B"], False, [-1.5, 4], [9, 23], "BV CMD for M52", False, 1)
        sourcer.ana_tricolour("M52_PREREDCATALOGUE", "phottable_table_prered", "_apflux_annuli_manu_atmos_scimag", ["V", "B", "U"], False, False, [0,2], [-6,3], False, "M52 UBV CC Plot Sample",4)
        ...
        # sourcer.ana_bicolour("GAIA", "test_query_2", "", ["g","bp"], False, False, [-1.5, 4], [23, 9], "GAIA bp-g CMD for NGC7789", False, 1)
        sourcer.ana_bicolour("NGC7789_PREREDCATALOGUE", "phottable_table_prered", "_apflux_annuli_manu_atmos_scimag", ["V","B"], False, [-1.5, 4], [9, 23], "BV CMD for NGC7789", False, 1)
        sourcer.ana_tricolour("NGC7789_PREREDCATALOGUE", "phottable_table_prered", "_apflux_annuli_manu_atmos_scimag", ["V", "B", "U"], False, False, [0,2], [-6,3], False, "NGC7789 UBV CC Plot Sample",1)
        ...
        sourcer.ana_bicolour("HIPPARCOS", "mag_table", "", ["vt", "bt"], False, [-0.5, 2.5], [-5, 15], "BV CMD for Hipparcos", "vtabs", 1)
        sourcer.ana_bicolour("HIPPARCOS", "mag_table_errored", "", ["vt", "bt"], False, [-0.5, 2.5], [-5, 15], "BV CMD for Hipparcos (Error Clipped)", "vtabs", 1)
    ...
    def turn_import(self):
        ...
        cols_own = ["ra", "dec", "pmra", "pmdec", "parallax", "parallax_error", "V_apflux_annuli_manu_atmos_scimag", "B_apflux_annuli_manu_atmos_scimag", "U_apflux_annuli_manu_atmos_scimag", "phot_g_mean_mag", "phot_bp_mean_mag"]
        ...
        for group in M52_groups:
            ...
            for i in table:
                ...
                V_abs.append(utilities.abs_to_app(i['V_apflux_annuli_manu_atmos_scimag'], 0, dist, disterr)[0])
                V_abs_dered.append(utilities.abs_to_app(i['V_dered'], 0, dist, disterr)[0])
            table['V_abs'] = V_abs
    ...
    def turn_bicolours(self):
        ...
        for num, group in enumerate(groups):
            turnanalysis.turn_bicolour_hipparcos(group, sets, "V_abs_dered", "BV_dered", True, False, False, "Dered test for " + group, 4, clusts[num])
    ...
    def turn_loglog(self):
        ...
        age_guess = "7.80", "9.14"
        metallicity_guess = "0.00", "-0.25"
        metallicities = ["-0.30"] # ["0.05", "0.00", "-0.05", "-0.20", "-0.25",
        maxshift = [0, 0]
        shift_increment = 0.02
        age_ranges = [[7.6, 8.18], [9.1, 9.30]]
        ...
        for metallicity in metallicities:
            for num, group in enumerate(groups):
                ...
                turnanal.turn_mspectype_estimator_UBV_CMD(group, set, absV, BV, lims_ubv[num], age_ranges[num], age_increment[num], metallicity)
        """
    ...
    def member(self):
        ...
        for group in groups:
            finder.pmscat(group, sets, 'pmra', 'pmdec', [[-3,0], [-3,0]])
            finder.radecscat(group, sets, 'ra', 'dec', False)
            finder.radcalc(group, sets, 12.5, 0.25, 0.3)
        ...
        """
        finder.m52second(groups[0], sets, 7, 0.3, 0.4)
        finder.pmscat2(groups[0], sets, "pmra", "pmdec")
        finder.radecscat2(groups[0], sets, "ra", "dec")
        newsets = "raw_data" + "_second_reduced"
        anal = astrowrapper.source_analysis(rootdir, "data.hdf5")
        anal.ana_bicolour(groups[0], newsets, "_apflux_annuli_manu_atmos_scimag", ["V", "B"], True, False, False,
                          "Test for " + groups[0] + "_second", False, 8)
        anal.ana_tricolour(groups[0], newsets, "_apflux_annuli_manu_atmos_scimag", ["V", "B", "U"], True, False, False, False,
                           False, "Test for " + groups[0] + "_second", 8) """
    def isofitter(self):
        groups = ["M52_Cross", "NGC7789_Cross"]
...
#mainer.source_photometry(["M52","NGC7789"], ["V","B","U"], 20, [["1031, 80", "3955, 3826"], ["120, 113", "3825, 3899"]], 3, 3, 3, 2.5, 3, 4, 20, True, 0, 0, 9999, 0.8, [200,200,200]) # sharplo, roundlo, sharphi, roundhi, minfluxes
