...
class orbifitter():
    ...
    def flatfork_least_squares_preliminary_orbit_fitting_thing_with_a_long_name(self,
        ...
        # Try to craft specific directory
        try:
            os.mkdir(windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_guess")
        except:
            pass

        if extra_text == None:
            savedir = windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_guess" + "\\" + str(clust_to_fit)
        else:
            savedir = windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_guess" + "\\" + str(
                clust_to_fit) + "_" + extra_text
    ...
    def finetune_least_squares_preliminary_orbit_fitting_thing_with_a_long_name(self,
                                                                                table,
                                                                                clust_to_fit,
                                                                                iterations,
                                                                                time_to_integrate,
        ...
        # Try to craft specific directory
        try:
            os.mkdir(windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_guess")
        except:
            pass

        if extra_text == None:
            savedir = windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_guess" + "\\" + str(
                clust_to_fit)
        else:
            savedir = windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_guess" + "\\" + str(
                clust_to_fit) + "_" + extra_text
        ...
        try:
            if try_load == True:
                with open(
                        windows_directories.orbitsdir + "\\" + (
                                "finetune_preliminary_fit_cluster_{0:.0f}_with_{1:.0f}_MCdraws_{2:.0f}_timesteps").format(
                            clust_to_fit,
                            iterations,
                            number_of_steps) + ".txt",
                        'rb') as f:
                ...
                with open(
                        windows_directories.orbitsdir + "\\" + (
                                "finetune_preliminary_fit_cluster_{0:.0f}_with_{1:.0f}_MCdraws_elements_{2:.0f}_timesteps").format(
                            clust_to_fit, iterations, number_of_steps)
                                .format(clust_to_fit, iterations) + ".txt", 'rb') as f:
                    orbit_elements = pickle.load(file=f)
                print("Orbits successfully loaded for " + str(clust_to_fit) + " with an iteration number of " + str(
        ...
        # Failed to load them. Generate them.
            ...
            try:
                ...
                raise ValueError(("{0},{1},DERP").format(data_to_fit, clustselec)) #meanvec/covtrix are NaN/broken.!!!
            # The error is that "clustselect" is ALL FALSE and "data_to_fit" is an EMPTY TABLE
        ...
        if graph == True:
            ...
            plt.xlabel('l')
            plt.ylabel('b')
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_finetune_all_orbits_testgalactic.png", dpi=300)
            plt.close()
    ...
    def finetune_galpy_final_preliminary(self,
                                         table,
                                         clust_to_fit,
                                         iterations,
                                         time_to_integrate,
        ...
        # Get an initial guess for the orbit with out code (will also produce plots for the orbit- see folder.)
        guess = self.finetune_least_squares_preliminary_orbit_fitting_thing_with_a_long_name(table,
                                                                                             clust_to_fit,
                                                                                             iterations,
                                                                                             time_to_integrate,
                                                                                             number_of_steps,
    ...
    def flatfork_galpy_final_fitting(self,
        ...
        if load_fit == False:
            ...
            if try_save == True:
                with open(
                        windows_directories.datadir + "\\" + "clustered_flatfork" + "\\" + str(clust_to_fit) + "_flatfork_memberpercent_galpy.orbit_fit.txt",
                        'wb') as f:
                    pickle.dump(obj=fit, file=f)
        else:
            ...
            try:
                ...
                with open(windows_directories.datadir + "\\" + "clustered_flatfork" + "\\" + str(clust_to_fit) + "_flatfork_memberpercent_galpy.orbit_fit.txt",
                          'rb') as f:
                    fit = pickle.load(file=f)
            ...
            # If it doesn't exist then get the fit and then save it! If try_save is true, obviously.
                ...
                if try_save == True:
                    with open(
                            windows_directories.datadir + "\\" + "clustered_flatfork" + "\\" + str(
        ...
        if graph == True:
            ...
            try:
                os.mkdir(windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables")
            except:
                pass
        ...
        if debug_graph != None:
            ...
            try:
                os.mkdir(windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_debugging")
            except:
                pass
            savedir = windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_debugging" + "\\" + str(clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
                pass
    ...
    def finetune_galpy_final_fitting(self,
                                     table,
                                     clust_to_fit,
                                     iterations,
                                     time_to_integrate,
        ...
        Galpy Final Fitting- finetune Rendition! :D
        ...
        if load_fit == False:
            ...
            if try_save == True:
                with open(
                        windows_directories.datadir + "\\" + "clustered_finetune" + "\\" + str(
                            clust_to_fit) + "_finetune_memberpercent_galpy.orbit_fit.txt",
                        'wb') as f:
                    pickle.dump(obj=fit, file=f)
        else:
            ...
            try:
                ...
                with open(windows_directories.datadir + "\\" + "clustered_finetune" + "\\" + str(
                        clust_to_fit) + "_finetune_memberpercent_galpy.orbit_fit.txt",
                          'rb') as f:
                    fit = pickle.load(file=f)
        ...
        if graph == True:
            ...
            try:
                ...
                pass
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_finetune_final_galpy_fit.png", dpi=300)
            plt.xlabel('l')
            plt.ylabel("b")
            plt.show()
        ...
        if debug_graph != None:
            ...
            try:
                os.mkdir(windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_debugging")
            except:
                pass
            savedir = windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_debugging" + "\\" + str(
                clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
            ...
            plt.xlabel('l')
            plt.ylabel("b")
            plt.savefig(savedir + "\\" + str(debug_graph) + "_finetune_final_galpy_fit.png", dpi=300)
            ...
            plt.ylabel(r'$\phi$')
            plt.grid(which='major', color='pink')
            plt.savefig(savedir + "\\" + str(debug_graph) + "_finetune_final_galpy_fit_RPHI.png", dpi=300)
        return fit
    ...
    def flatfork_galpy_fitting_nomemb(self,
        ...
        if graph == True:
            ...
            try:
                ...
                pass
            if extra_text == None:
                savedir = windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit)
            else:
                savedir = windows_directories.imgdir + "\\" + "flatfork_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit) + "_" + extra_text
    ...
    def finetune_galpy_fitting_nomemb(self,
                                      table,
                                      clustering,
                                      clust_to_fit,
                                      iterations,
                                      ...
                                      load_fit=False,
                                      try_save=False,
                                      extra_text="finetune_maindata_guess"):
        """
        :param table: the data table
        :param clustering: the clustering
        :param clust_to_fit: Which cluster do you want to fit from percent_table_greatfitted
        ...
        if load_fit == True:
            try:
                # Load the fit
                with open(windows_directories.datadir + "\\" + "clustered_finetune" + "\\" + str(
                        clust_to_fit) + "_finetune_maindata_galpy.orbit_fit.txt",
                          'rb') as f:
                    fit = pickle.load(file=f)
            except:
                # Run and save the fit
        ...
        if graph == True:
            ...
            try:
                ...
                pass
            if extra_text == None:
                savedir = windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit)
            else:
                savedir = windows_directories.imgdir + "\\" + "finetune_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit) + "_" + extra_text
            ...
            plt.xlabel('l')
            plt.ylabel("b")
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_finetune_maindata_galpy_fit.png", dpi=300)
            plt.close()
            plt.clf()
