...
from energistics_new_constants import M_d, M_b, M_nfw, a_b, a_d, b_d, a_nfw, c_nfw
...
def do_monte_table(groupset):
    ...
    # Set up Monte
    sourcedir = windows_directories_new.sourcedir
...
def do_duplimonte_table(groupsetm):
    ...
    list_of_columns, list_of_columns_2, list_of_columns_3, list_of_columns_4 = galcentricutils_new.monte_angular().panda_duplimonte(df, m) # , list_of_columns_2, list_of_columns_3, list_of_columns_4
    ...
    for column, savedex in zip(list_of_columns, indices):
        with open(os.path.join(os.path.join(windows_directories_new.duplimontedir,group),savedex), 'wb') as f:
    ...
    for column, savedex in zip(list_of_columns_2, indices_2):
        with open(windows_directories_new.duplimontedir + "\\" + group + "\\" + savedex, 'wb') as f:
            pickle.dump(column, f)
...
def do_hdbscan(arrayinfominpar):
    ...
    with open(windows_directories_new.duplimontedir + "\\" + arrayinfo[0] + "\\" + arrayinfo[1] + ".txt", 'rb') as f:
        data = pickle.load(f)
    data = np.array(data)
    clustered = galcentricutils_new.cluster3d().listhdbs(data, minpar)
    with open(windows_directories_new.duplimontedir + "\\" + arrayinfo[0] + "\\" + arrayinfo[1] + ".cluster.txt", 'wb') as f:
    ...
    graphutils_new.threed_graph().kmeans_L_array(data, clustered, savedexdir, False)
...
def flatfork_do_hdbscan(arrayinfominpar):
    ...
    with open(windows_directories_new.clusterer_flat_path, "rb") as f:
        clusterer = pickle.load(file=f)

    # Get parameters/etc + load data
    arrayinfo, minpar = arrayinfominpar
    with open(os.path.join(os.path.join(windows_directories_new.duplimontedir,
                                        arrayinfo[0]),
                           arrayinfo[1] + ".txt"),
              'rb') as f:
        data = pickle.load(f)
...
def do_hdbscan_L4D(arrayinfominpar):
    ...
    graphutils_new.threed_graph().kmeans_L_array(data, clustered, savedexdir, False, False)
...
def do_hdbscan_greatfit(arrayinfominpar):
    ...
    greatcount = galcentricutils_new.greatcount()
    clust3d = galcentricutils_new.cluster3d()
    compclust = galcentricutils_new.compclust()

    # Optionally graph
    #grapher = graphutils_new.threed_graph()
...
def do_updated_hdbscan_greatfit(arrayinfominpar):
    ...
    # Load in the data
    with open(windows_directories_new.duplimontedir + "\\" + group_saveid[0] + "\\" + group_saveid[1] + ".txt", 'rb') as f:
        data = pickle.load(f)
    data = np.array(data) # as vectors, i.e. 6 columns and len(data) rows. Lx Ly Lz x y z.
    ...
    # Load in the average data, for the sake of "remapping" this data (saves us some effort later.)
    with open(windows_directories_new.clusterdir + "\\" + "flatfork_fullgroup_cluster.txt", 'rb') as f:
    ...
    for cluster, best_minpar, greatpar in zip(clustrange, best_minpars, greatpars):
        ...
        is_gse = np.array([True if d == ascii_info_new.flatfork_GSE_ID else False for d in cut_clustered])
        ...
        # Save the remap
        with open(windows_directories_new.duplimontedir + "\\" + group_saveid[0] +
...
# Fit an orbit with the energistics_new orbifitter using Galpy, then save the fit, for the Monte-Carlo Data Iterations
...
def do_orbifit(parameters):
    ...
    # Set up the fitter
    fitter = energistics_new.orbifitter()
    ...
    for clust_to_fit in clusters_to_cluster:
        ...
        # Save it
        with open(windows_directories_new.orbitsfitdir + "\\" + arrayinfo[0] + "_" +
...
def maindata_do_orbistatistics(clust_to_fit):
    ...
    savedir = os.path.join(os.path.join(windows_directories_new.imgdir,"orbit_fitting_variables_maindata"),str(clust_to_fit) + "_orbifit_maindata_run")
    ...
    for saveid in ascii_info_new.flatfork_orbifit_maindata_saveids:

        # Load it
        with open(windows_directories_new.orbitsfitdir + "\\" + group + "_" +
    ...
    twod_graph = graphutils_new.twod_graph()
...
def flatfork_maindata_do_orbistatistics(clust_to_fit):
    ...
    # Specify savedir/savename and make path, for images
    savedir = os.path.join(os.path.join(windows_directories_new.imgdir,"flatfork_orbit_fitting_variables_maindata"),str(clust_to_fit) + "_orbifit_maindata_run")
    save_unique = str(clust_to_fit)
    try:
        os.mkdir(savedir)
    except:
...
def do_orbistatistics(clust_to_fit):
    ...
    # In n_carlo
    group = ascii_info_new.fullgroup
    saveids = ascii_info_new.orbifit_saveids
...
def flatfork_greatcircle_optimize(parameterss):
    ...
    try:
        graphutils_new.spec_graph().clust_thetaphi(table=table_included, clustering=clustered_included,
...
def flatfork_do_finetune(args):
    ...
    from galcentricutils_new import compclust, cluster3d
    cc = compclust()
    c3d = cluster3d()
    ...
    graphutils_new.threed_graph().kmeans_L_array(cut_array, cut_clustered,
    ...
    graphutils_new.threed_graph().kmeans_L_array(cut_array, trial_clustered,
