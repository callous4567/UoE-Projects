...
class twod_graph(object):
    ...
    def tripL(self, table, maxradius, divisor=1e3):
        table = galcentricutils_new.angular().get_momentum(table)
        ...
        plt.savefig(windows_directories_new.imgdir + "\\test.png", dpi=300, transparent="True", pad_inches=0.1)
        plt.show()
    ...
    def haitoff(self, table):
        ...
        table = galcentricutils_new.angular().get_latipolar(table)
    ...
    def latipolar(self, table, theta_polar, phi):
        ...
        gcc = galcentricutils_new.greatfit().gcc_gen(10000, theta_polar, phi) # thetas, phis
    ...
    def thetaphi_twodhist(self, table, nbins, savedex, latipolar):
        ...
        try:
            os.mkdir(windows_directories_new.imgdir + "\\thetaphi_twodhist")
        except:
            pass

        plt.savefig(windows_directories_new.imgdir + "\\thetaphi_twodhist" + "\\" + savedex + ".png")
        plt.show()

    # Fancy l,b plot
    def lbplot(self, table, savepath=False, negpi=False):
    ...
    def nclust_n(self, data, group):
        ...
        plt.savefig(windows_directories_new.imgdir + "\\graph_nclust.png", dpi=300)
        plt.show()

    # Create a very fancy histogram plot for the data! (PDF.)
    def hist_fancy(self, data, nbins, xlabel, ylabel, savepath=None, dencurve=True, lims=None):
    ...
    def corr_plot(self, val1, val2, val1err, val2err, xlabel, ylabel, savepath=None, lims=None):
        ...
        if lims == None:
            mean, med, stdev = sigma_clipped_stats(val1)
            axs.set(xlim=[mean - 3*stdev,
                          mean + 3*stdev])
            mean, med, stdev = sigma_clipped_stats(val2)
            axs.set(ylim=[mean - 3*stdev,
                          mean + 3*stdev])
...
class threed_graph(object):
    ...
    def kmeans_L(self, table, savedex, browser, divisor=1e3):
        ...
        plt.savefig(windows_directories_new.imgdir + "\\test3d.png", dpi=300)
        plt.show()
        ...
        try:
            os.mkdir(windows_directories_new.imgdir + "\\kmeans_html")
        except:
            pass
        ...
        if savedex != False:
            save_loc = windows_directories_new.imgdir + "\\kmeans_html" + "\\" + savedex
    ...
    def xmeans_L(self, table, savedex, browser, divisor=1e3):
        ...
        # Also generate an interactive 3D plot. This is mainly for debug purposes.
        try:
            os.mkdir(windows_directories_new.imgdir + "\\xmeans_html")
        except:
            pass

        table_pandas = table.to_pandas()
        fig = px.scatter_3d(table_pandas, x='x', y='y', z='z', color='k_index', title=savedex)
        save_loc = windows_directories_new.imgdir + "\\xmeans_html" + "\\" + savedex
        fig.write_html(save_loc)
        if browser == True:
            fig.show()
    ...
    def xmeans_L_array(self, array, clusterdata, savedexdir, browser, outliers=False, omit=None):
        ...
        if savedexdir != False :
            save_loc = windows_directories_new.imgdir + "\\" + savedexdir + ".html"
            fig.write_html(save_loc)
        if browser == True:
            fig.show()
    ...
    # Except it takes arrays. Savedexdir here should be the directory\\savename
    def kmeans_L_array(self, array, clusterdata, savedexdir, browser, outliers=False, limit_view=True):
        """
        Important note: savedexdir is used via...

            save_loc = os.path.join(windows_directories_new.imgdir,savedexdir + ".html")
            fig.write_html(save_loc)
        ...
        if outliers == False:
            ...
            if limit_view == True:
                array_clip_indices = galcentricutils_new.cluster3d().getCuboid(array)
        ...
        if browser == True:
            fig.show()
    def kmeans_L_array_path(self, array, clusterdata, savepath, browser, outliers=False, limit_view=True):
...
class spec_graph(object):
    ...
    def clust_radec(self, table, clustering,
        ...
        if cluster_id is False:
            ...
            if vasiliev==True:
                simdir = windows_directories_new.datadir + "\\vasiliev\\Sgr_snapshot"
            ...
            table = galcentricutils_new.galconversion().nowrite_GAL_to_ICRS(table)
            ...
            if savedexdir != None and flatfork == False:
                plt.savefig(os.path.join(windows_directories_new.imgdir + "\\" + "vasiliev",savedexdir + ".png"), dpi=300)
            if savedexdir != None and flatfork == True:
                plt.savefig(os.path.join(windows_directories_new.imgdir + "\\" + "flatfork_vasiliev",savedexdir + ".png"), dpi=300)
            ...
            try:
                ...
                # Then our clustering...
                masked_table = galcentricutils_new.galconversion().nowrite_GAL_to_ICRS(masked_table)
                ...
                # Create save directory/save
                if savedexdir != None and flatfork == False:
                    plt.savefig(os.path.join(windows_directories_new.imgdir + "\\" + "vasiliev", savedexdir + ".png"),
                                dpi=300)
                    plt.close()
                    plt.clf()
                elif savedexdir != None and flatfork == True:
                    plt.savefig(
                        os.path.join(windows_directories_new.imgdir + "\\" + "flatfork_vasiliev", savedexdir + ".png"),
                        dpi=300)
                    plt.close()
                    plt.clf()
    ...
    def clust_thetaphi(self, table, clustering, cluster_id=False, vasiliev=False, savedexdir=None, gcc_thetaphis=None,
        ...
        if cluster_id is False:
            ...
            if vasiliev==True:
                ...
                sim_table = galcentricutils_new.angular().get_polar(sim_table)
            ...
            if savedexdir != None and flatfork == False:
                try:
                    os.mkdir(windows_directories_new.imgdir + "\\" + "vasiliev")
                except:
                    pass
                plt.savefig(windows_directories_new.imgdir + "\\" + "vasiliev" + "\\" + savedexdir + "_thetaphi.png", dpi=300)
            if savedexdir != None and flatfork == True:
                try:
                    os.mkdir(windows_directories_new.imgdir + "\\" + "flatfork_vasiliev")
                except:
                    pass
                plt.savefig(windows_directories_new.imgdir + "\\" + "flatfork_vasiliev" + "\\" + savedexdir + "_thetaphi.png", dpi=300)
            # Show
            #plt.show()
    ...
    def array_thetaphi_debug(self, array, theta, phi, clusters, whichclust):
        # Get angular
        ang = galcentricutils_new.angular()
        # Get polars
        ...
        legend_elements.append(Patch(edgecolor='white',
                                     ...
                                     label="In Cluster"))
        # We can also add a great circle (though this necessitates a legend.)
        gccthetas, gccphis = galcentricutils_new.greatfit().gcc_gen(1000, theta, phi)
        axs.scatter(gccthetas, gccphis, color="blue", s=0.1)
        legend_elements.append(Patch(edgecolor='white',facecolor='blue',label='GCC Fit'))
        plt.legend(loc='upper right', handles=legend_elements)
        axs.set_facecolor("k")
    ...
    def sofie(self, table):
        ...
        grid.save(windows_directories_new.imgdir + "\\" + 'sixplot.jpg')
    ...
    def energy_plot(self, table, clustering, clusters, texts, x_lim, y_lim, savepath=None, mcmillan=False, kpcmyr=False):
        ...
        import energistics_new
        if mcmillan == False:
            energist = energistics_new.fast_energistics_new()
            table = energist.default_E_c(table)
        else:
            # Calculate energy via McMillan 2017
            energist = energistics_new.energistics_new()
