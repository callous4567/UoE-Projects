...
from energistics_new_constants import a_b, a_d, b_d, a_nfw, M_b, M_d, M_nfw, c_nfw
...
# Get E-Factor for fast-energistics_new
...
class energistics_new(object):
    def __init__(self):
        # Grab the parameters of the solar system in the galactocentric frame- use galcentricutils_new.
        self.converter = galcentricutils_new.galconversion()
...
"""
energistics_new but manually defined- see notes.
...
class energistics_new_manual(object):
    def __init__(self):
        # Grab the parameters of the solar system in the galactocentric frame- use galcentricutils_new.
        converter = galcentricutils_new.galconversion()
        converter.solinfo_grab(windows_directories_new.sourcedir, "solar_info.dat")
        """
        Galpy converts everything into internal units (radii and velocities.) 
        The scale is ro and vo- radius of sol galcentrically, and velocity.
        We're going to use ro as the x-radius in galcentric (cylindrical radius)
        ...
        # Set up fast
        self.fast = fast_energistics_new()
    ...
    def pot_eval(self, table, savedexdir):
        ...
        # Als evaluate and overplot using automatic energistics_new
        table_orig = energistics_new().pot_eval(table)
        plt.scatter(table_orig['Lz'], table_orig['energy'], marker="x", s=0.5, color='red')

        # Save and show.
        if savedexdir != None:
            plt.savefig(windows_directories_new.imgdir + savedexdir + ".png", dpi=600)
        plt.show(dpi=300)
...
"""
Numba-fied version of energistics_new_manual, for fast potential calculations in entropy plotting.
...
class fast_energistics_new(object):
    def __init__(self):
        """
        This is the set of constants for energistics_new to model the bulge, disk, and halo.
        ...
        energistics_new will use these in calculating amplitudes and etc for using GALPY.

        You should note that energistics_new introduces astropy units/quantities on top of these-
    ...
    def default_E_c(self, table):
        ...
        try:
            ...
            # Float64 error, probably.
            table['x'], table['y'], table['z'], \
            table['vx'], table['vy'], table['vz'] = np.array(table['x'], np.float64), \
                                                    np.array(table['y'], np.float64), \
                                                    np.array(table['z'], np.float64), \
                                                    np.array(table['vx'], np.float64), \
                                                    np.array(table['vy'], np.float64),\
                                                    np.array(table['vz'], np.float64)
    ...
    # With a return just on the energy
    def default_E_c_vals(self, table):

        try:

            E = self.E_c(table['x'], table['y'], table['z'], table['vx'], table['vy'], table['vz'],
                                  M_b, a_b, M_d, a_d, b_d, M_nfw, a_nfw, c_nfw)
            return E
            ...
            E = self.E_c(table['x'], table['y'], table['z'], table['vx'], table['vy'], table['vz'],
                                  M_b, a_b, M_d, a_d, b_d, M_nfw, a_nfw, c_nfw)
            return E
# Class for dealing with galpy orbits. Inherits from energistics_new.
# See https://docs.galpy.org/en/v1.7.1/reference/orbit.html for information on handling orbits.
class orbigistics(energistics_new):
    def __init__(self):
        energistics_new.__init__(self)
        self.fast = fast_energistics_new()
    ...
    def get_leftgalpy(self, table):
        ...
        z, \
        vz, \
        phi = galcentricutils_new.angular().left_numba_cylindrical(data_array)  # phi in radians.
...
class orbifitter():
    def __init__(self):
        self.ang = galcentricutils_new.angular()
    ...
    def least_squares_preliminary_orbit_fitting_thing_with_a_long_name(self,
        ...
        try:
            os.mkdir(windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_guess")
        except:
            pass

        if extra_text == None:
            savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_guess" + "\\" + str(clust_to_fit)
        else:
            savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_guess" + "\\" + str(
        ...
        try:
            if try_load == True:
                with open(
                        windows_directories_new.orbitsdir + "\\" + (
        ...
        # Failed to load them. Generate them.
            ...
            for element in orbit_elements:
                ...
                table = galcentricutils_new.angular().get_polar(table)  # get the fit in galactocentric polars
            ...
            if try_save == True:
                try:
                    with open(windows_directories_new.orbitsdir + "\\" + (
                            "preliminary_fit_cluster_{0:.0f}_with_{1:.0f}_MCdraws_{2:.0f}_timesteps").format(
                        clust_to_fit,
                        iterations,
                        number_of_steps) + ".txt",
    ...
    # TODO: Flatfork this
    def flatfork_least_squares_preliminary_orbit_fitting_thing_with_a_long_name(self,
                                                                       table,
                                                                       clust_to_fit,
                                                                       iterations,
                                                                       time_to_integrate,
        ...
        # Try to craft specific directory
        try:
            os.mkdir(windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_guess")
        except:
            pass

        if extra_text == None:
            savedir = windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_guess" + "\\" + str(clust_to_fit)
        else:
            savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_guess" + "\\" + str(
                clust_to_fit) + "_" + extra_text
        ...
        try:
            if try_load == True:
                with open(
                        windows_directories_new.orbitsdir + "\\" + (
                                "flatfork_preliminary_fit_cluster_{0:.0f}_with_{1:.0f}_MCdraws_{2:.0f}_timesteps").format(
                            clust_to_fit,
                            iterations,
                            number_of_steps) + ".txt",
                        'rb') as f:
                ...
                with open(
                        windows_directories_new.orbitsdir + "\\" + (
                                "flatfork_preliminary_fit_cluster_{0:.0f}_with_{1:.0f}_MCdraws_elements_{2:.0f}_timesteps").format(
                            clust_to_fit, iterations, number_of_steps)
                                .format(clust_to_fit, iterations) + ".txt", 'rb') as f:
                    orbit_elements = pickle.load(file=f)
                print("Orbits successfully loaded for " + str(clust_to_fit) + " with an iteration number of " + str(
        ...
        if graph == True:
            ...
            plt.xlabel('l')
            plt.ylabel('b')
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_flatfork_all_orbits_testgalactic.png", dpi=300)
            plt.close()
    ...
    def galpy_final_fitting(self,
        ...
        :param debug_graph: bool. Should I produce debug graphs? If yes, then set this to a unique identity string.
        ...
        try:
            ...
            if load_fit == True:
                ...
                with open(
                        windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(clust_to_fit) + ".orbit_fit.txt",
                        'rb') as f:
                    fit = pickle.load(file=f)

            else:

                raise ValueError("Generating orbits and not loading...")
        ...
        if graph == True:
            ...
            # r, theta, phi = np.array(galcentricutils_new.angular().right_numba_polar(R, z, phi)).T
            ...
            try:
                os.mkdir(windows_directories_new.imgdir + "\\" + "orbit_fitting_variables")
            except:
                pass
            savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables" + "\\" + str(clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
                pass
        ...
        if debug_graph != None:
            ...
            try:
                os.mkdir(windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_debugging")
            except:
                pass
            savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_debugging" + "\\" + str(clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
                pass
    ...
    def flatfork_galpy_final_preliminary(self,
                            table,
                            clust_to_fit,
                            iterations,
                            time_to_integrate,
        ...
        Galpy Final Fitting- Produce the preliminary guess.
        ...
        # Get an initial guess for the orbit with out code (will also produce plots for the orbit- see folder.)
        guess = self.flatfork_least_squares_preliminary_orbit_fitting_thing_with_a_long_name(table,
                                                                                    clust_to_fit,
                                                                                    iterations,
                                                                                    time_to_integrate,
                                                                                    number_of_steps,
    ...
    def flatfork_galpy_final_fitting(self,
                            table,
                            clust_to_fit,
                            iterations,
                            time_to_integrate,
        ...
        Galpy Final Fitting- Flatfork Rendition! :D
        ...
        if load_fit == False:
            ...
            if try_save == True:
                with open(
                        windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(clust_to_fit) + "_flatfork_memberpercent_galpy.orbit_fit.txt",
                        'wb') as f:
                    pickle.dump(obj=fit, file=f)
        else:
            ...
            try:
                ...
                with open(windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(clust_to_fit) + "_flatfork_memberpercent_galpy.orbit_fit.txt",
                          'rb') as f:
                    fit = pickle.load(file=f)

            # If it doesn't exist then get the fit and then save it! If try_save is true, obviously.
            except:
                ...
                if try_save == True:
                    with open(
                            windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(
                                clust_to_fit) + "_flatfork_memberpercent_galpy.orbit_fit.txt",
                            'wb') as f:
                        pickle.dump(obj=fit, file=f)
        ...
        if graph == True:
            ...
            try:
                os.mkdir(windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables")
            except:
                pass
            savedir = windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables" + "\\" + str(clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
                pass
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_flatfork_final_galpy_fit.png", dpi=300)
            plt.xlabel('l')
            plt.ylabel("b")
            plt.show()
        ...
        if debug_graph != None:
            ...
            try:
                os.mkdir(windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_debugging")
            except:
                pass
            savedir = windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_debugging" + "\\" + str(clust_to_fit)
            try:
                os.mkdir(savedir)
            except:
                pass
            ...
            plt.xlabel('l')
            plt.ylabel("b")
            plt.savefig(savedir + "\\" + str(debug_graph) + "_flatfork_final_galpy_fit.png", dpi=300)
            ...
            plt.ylabel(r'$\phi$')
            plt.grid(which='major', color='pink')
            plt.savefig(savedir + "\\" + str(debug_graph) + "_flatfork_final_galpy_fit_RPHI.png", dpi=300)
        return fit
    ...
    def galpy_fitting_nomemb(self,
        ...
        if load_fit == True:
            try:
                # Load the fit
                with open(windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(
        ...
        if graph == True:
            ...
            try:
                ...
                pass
            if extra_text == None:
                savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit)
            else:
                savedir = windows_directories_new.imgdir + "\\" + "orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit) + "_" + extra_text
    ...
    def flatfork_galpy_fitting_nomemb(self,
                             table,
                             clustering,
                             clust_to_fit,
                             iterations,
                             ...
                             load_fit=False,
                             try_save=False,
                             extra_text="flatfork_maindata_guess"):
        """
        :param table: the data table
        :param clustering: the clustering
        :param clust_to_fit: Which cluster do you want to fit from percent_table_greatfitted
        ...
        if load_fit == True:
            try:
                # Load the fit
                with open(windows_directories_new.datadir + "\\" + "clustered" + "\\" + str(
                        clust_to_fit) + "_flatfork_maindata_galpy.orbit_fit.txt",
                          'rb') as f:
                    fit = pickle.load(file=f)
            except:
                # Run and save the fit
        ...
        if graph == True:
            ...
            try:
                ...
                pass
            if extra_text == None:
                savedir = windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit)
            else:
                savedir = windows_directories_new.imgdir + "\\" + "flatfork_orbit_fitting_variables_maindata" + "\\" + str(
                    clust_to_fit) + "_" + extra_text
            ...
            plt.xlabel('l')
            plt.ylabel("b")
            plt.savefig(savedir + "\\" + str(clust_to_fit) + "_flatfork_maindata_galpy_fit.png", dpi=300)
            plt.close()
            plt.clf()
...
class entro_fitter():
    ...
    def run_grid(self, print_it=False):
        ...
        entgrid = self.coarse_entropy(True, windows_directories_new.imgdir + "\\orphan_entropy_test.png")
