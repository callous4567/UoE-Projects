...
from ascii_info_new import n_carlo
import pickle
...
# The saveids
saveids = ascii_info_new.flatfork_orbifit_maindata_saveids
...
if __name__ == "__main__" and should_run == True:
    ...
    # Run the pool!
    pool = multiprocessing.Pool(8)
    results = pool.map(windows_multiprocessing_new.flatfork_do_orbifit_maindata, parameterss)
    pool.close()
...
if should_run == False and __name__ == "__main__": # False:
    ...
    # Run the pool!
    pool = multiprocessing.Pool(8)
    list_of_stats = pool.map(windows_multiprocessing_new.flatfork_maindata_do_orbistatistics, clusters_to_orbifit)
    pool.close()
