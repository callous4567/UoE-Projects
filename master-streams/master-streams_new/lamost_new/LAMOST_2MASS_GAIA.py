...
from APOGEE import apogee_starhorse_path_nogcs
from energistics_new import fast_energistics_new

"""
A value significantly greater than 1.0 (say, >1.4) 
could indicate that the source is non-single or otherwise 
problematic for the astrometric solution.
"""

# Should I import the provided "Distant Gaia" dataset? NOW HAS RUWE < 1.4
distant_gaia = False
distant_gaia_dir = os.path.join(windows_directories_new.lamodir, "gaia_vots")
distant_gaia_path = os.path.join(windows_directories_new.lamodir, "distant_gaia.vot")
"""

You'll note that we 
a) Don't have much Sagittarius Stream
b) Don't have much else
Dataset isn't that great.

SELECT ra, dec, parallax, parallax_error, pmra, pmra_error, pmdec, pmdec_error, radial_velocity, radial_velocity_error, source_id, parallax_over_error
...
This is the code for taking the Gaia queries (manually done) + combining them up 
We've got all the Gaia stars down to an error/parallax ratio of unity here.
"""

if __name__ == "__main__" and distant_gaia == True:

    # Get all the distant_gaia vots from the directory
    paths = os.listdir(distant_gaia_dir)
    paths = [os.path.join(distant_gaia_dir, path) for path in paths]
    vot = []
    for path in paths:
        subvot = votable.parse_single_table(path).to_table()
        vot.append(subvot)

    # Stack them all
    vot = vstack(vot)

    # Process columns to match our standard use
    vot.rename_column("radial_velocity", "vlos")
    vot.rename_column("radial_velocity_error", "evlost")
    vot['dist'] = 1/vot['parallax']
    vot['edist'] = vot['dist'] * vot['parallax_error']/vot['parallax']
    ...
    # Get Galcent
    vot = orbigist.converter.nowrite_ICRS_to_GAL(vot, has_cosfactor=True)
    vot = orbigist.converter.nowrite_GAL_to_GALCENT(vot)
    vot = angular().get_momentum(vot)

    clip = [True if r > 15 else False for r in vot['r']]
    vot = vot[clip]
    print("The length of our trimmed distant_gaia beyond radius 15 kpc is given by ", len(vot))
    ...
    import gcc_utils
    gc_to_remove = gcc_utils.remove_gcs(np.array(vot['x'], float), np.array(vot['y'], float),
                                        np.array(vot['z'], float),
                                        np.array(gc_table['x'], float), np.array(gc_table['y'], float),
                                        np.array(gc_table['z'], float),
                                        np.array(gc_table['rt'], float))
    vot = vot[gc_to_remove]

    vot.write(windows_directories_new.gaia_distantmatch_galcent, overwrite=True)

    #data = np.array([vot['Lx'], vot['Ly'], vot['Lz']]).T
...
# Should I process the VAT for DR7? Note that this is for DR7 V1 apparently (the V2 measures don't add up??? :/)
process_vat = False
...
# Should I crossmatch the 2MASS-LAMOST with Gaia dataset? Note- we've included ruwe < 1.4 pruning now, too.
LAMOST_2MASS = False
if __name__ == "__main__" and LAMOST_2MASS == True:
    ...
    # Get stars a minimum of 4 kpc from us, Earth
...
if __name__ == "__main__" and do_galcent_gcs == True:
    ...
    if produce_data:
        ...
        #data = orbigist.orbilarity(data)

        # Save fits and ascii.
        data.write(final_dir, overwrite=True)
        #data.write(final_ascii, format='ascii', overwrite=True)
...
# Also do monte for galactic errors for LAMOST catalogue
# Will also remove nans/etc from the data (where present!) such that the rest of the data isnt compromised in process
# TODO: Metrics for matching to old catalogue
do_finalstack = False
finalstack_load = True
