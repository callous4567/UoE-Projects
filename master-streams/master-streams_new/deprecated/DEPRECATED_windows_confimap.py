...
for dtheta in dthetarange:
    for n_phi in nphiarange:
        ...
        n_points = 2.4e4
        coordgrid = galcentricutils_new.greatcount().fullgridgen(n_points) # IN RADIANS!
        #graphutils_new.threed_graph().unitsphere(coordgrid[0], coordgrid[1], coordgrid[2])
        ...
        def main():
            if __name__ == '__main__':
                ...
                result = pool.map(func=windows_multiprocessing_new.do_gccsplit_confidence, iterable=mapped_variables)
                pool.close()
                return result
