...
for dtheta in windows_testing.dthetarange:
    for n_phi in windows_testing.nphiarange:
        try:
            ...
            # polar_data = galcentricutils_new.angular().get_polar(table_data)
            table = writer.read_table(ascii_info_new.fullgroup, string_format)
            ...
            try:
                os.mkdir(windows_directories_new.imgdir + "\\" + "confidence_map")
            except:
                pass

            plt.savefig(windows_directories_new.imgdir + "\\" + "confidence_map" + "\\" + string_format + ".png")
            plt.clf()
            plt.close()

            # Quick number density plot
            # graphutils_new.twod_graph().thetaphi_twodhist(polar_data, 200)
        except:
            pass
