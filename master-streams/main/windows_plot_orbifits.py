...
if do_lbplots == True:
    savedir = os.path.join(windows_directories.imgdir, "lb_plots")
    try:
        os.mkdir(savedir)
    except:
        pass
    for clust in clusters_to_orbifit:
        ...
        # Grab an orbit fit
        orbifit = energistics.orbifitter().galpy_final_fitting(table,
                                                                        clust,
                                                                        2000,
                                                                        0.3e9,
                                                                        1000,
...
if do_energyplots == True:
    ...
    savedir = os.path.join(windows_directories.imgdir, "energy_plots")
    try:
        os.mkdir(savedir)
    except:
        pass
    prelim_path = os.path.join(savedir, "streamplot_prelim.png")
    final_path = os.path.join(savedir, "streamplot_final.png")
...
if do_clustplots == True:
    ...
    savedir = os.path.join(windows_directories.imgdir, "clustplots")
    try:
        os.mkdir(savedir)
    except:
        pass
