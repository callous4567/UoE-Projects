...
for cluster, M_d, M_nfw, c_nfw in zip(entropy_table['cluster'], entropy_table['M_d'], entropy_table['M_nfw'], entropy_table['c_nfw']):
    ...
    if cluster == 4:
        ...
        m_vals = [orbigist.ret_halo_disk_manudisk(r, M_d*energistics_constants.M_d, M_nfw*energistics_constants.M_nfw, c_nfw*energistics_constants.c_nfw) for r in r_vals]
...
plt.savefig(windows_directories.imgdir + "\\entropy_oned_halo", dpi=300)
plt.show()
plt.close()
