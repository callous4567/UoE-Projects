...
#clusters_to_orbifit = ascii_info.finetune_clusters_to_maindata_orbifit
...
if __name__ == "__main__" and should_run == True:
    ...
    pool = multiprocessing.Pool(8)
    results = pool.map(windows_multiprocessing.finetune_do_orbifit, parameterss)
    pool.close()
...
if should_run == False and __name__ == "__main__": # False:
    ...
    # Run the pool!
    pool = multiprocessing.Pool(8)
    list_of_stats = pool.map(windows_multiprocessing.finetune_do_orbistatistics, clusters_to_orbifit)
    pool.close()
