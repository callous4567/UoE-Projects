...
if just_plot == False and __name__ == "__main__":
    ...
    # Get the unique clusters in this (save noise and the GSE.)
    clusters_to_try = []
    ...
    # Make the zip required for us (to cast the greatcircle optimization for multiprocessing.)
    zipped = []
    ...
    results = pool.map(windows_multiprocessing.finetune_greatcircle_optimize, zipped)
    pool.close()
...
if just_plot == True and __name__ == "__main__":
    ...
    greatcircle_table = hdfutils.hdf5_writer(windows_directories.datadir,
                                             ascii_info.finetune_asciiname).read_table(
        "greatcircles_nonmemberpercent_preliminary",
        "greatcircles")

    # Get the unique clusters in this (save noise and GSE.)
    clusters_to_try = []
    for clust in list(set(clustered)):
        if clust not in clusters_to_try:
            if clust != -1:
    ...
    for which_try in clusters_to_try:
        ...
        try:
            graphutils.spec_graph().clust_thetaphi(table=table_included, clustering=clustered[included],
                                                   ...
                                                   vasiliev=False, savedexdir="finetune_greatcount_beforeMC_" + str(
