...
total_percent_table = hdfutils.hdf5_writer(windows_directories.datadir,
                             ascii_info.finetune_asciiname).read_table(ascii_info.fullgroup,
...
if do_lbplots == True:
    ...
    savedir = os.path.join(windows_directories.imgdir, "finetune_lb_plots")
    ...
    for clust in clusters_to_orbifit:
        ...
        # Grab an orbit fit
        orbifit = energistics.orbifitter().finetune_galpy_final_fitting(table,
                                                                        clust,
                                                                        2000,
                                                                        0.3e9,
                                                                        1000,
...
if do_energyplots == True:
    ...
    savedir = os.path.join(windows_directories.imgdir, "finetune_energy_plots")
    try:
        os.mkdir(savedir)
    except:
        pass
    prelim_path = os.path.join(savedir, "finetune_streamplot_prelim.png")
    final_path = os.path.join(savedir, "finetune_streamplot_final.png")
...
if do_clustplots == True:
    ...
    savedir = os.path.join(windows_directories.imgdir, "finetune_clustplots")
    try:
        os.mkdir(savedir)
    except:
        pass
    ...
    graphutils.twod_graph().tripL_colour(tablee, 15, clustering_final, os.path.join(savedir, "tripL_final.png"))
    ...
    graphutils.twod_graph().tripL_colour(tablee, 15, clustering_prelim, os.path.join(savedir, "tripL_prelim.png"))
